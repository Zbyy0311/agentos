...
> **Branch:** `runtime/m2-6-idempotency-concurrency` (original M2.6 implementation and verification branch, retained after merge), current main `6727add8303b7d0ab659a427bfdd8299a98e5702`
...
| M2.7 | CURRENT-STATE AUDIT AND PLANNING IN PROGRESS — IMPLEMENTATION NOT AUTHORIZED | Branch `runtime/m2-7-v1-compatibility-migration`; base `30b3b20c9c4239fdfc9d423497b04c94e9468dde`; audit, Owner Decision draft, and plan draft are authoritative; no PR; M2.8 not started |
...
#### Verification Closure (2026-07-24 — HISTORICAL 时点记录)
...
> **M2.6 override (2026-07-28, Historical pre-merge plan context):** the detailed M2.6 draft above is pre-M2.5 history and is **not** implementation authorization. Historical pre-merge facts: the migration registry was TS-based 001–009 (M2.5 consumed 007–009), so `v008-idempotency-records.sql` was stale and the next migration candidate was **010** (exact schema frozen in OD-18); `routes/tasks.ts`/`routes/runs.ts` were Legacy/Conversation surfaces and did not constitute implementation authority — the canonical Task-domain API was the workspace-scoped v2 routes; optimistic version infrastructure was partially implemented (Repository version guards); Approval was an in-memory Registry and deferred by OD-17. Owner Decisions OD-1–OD-20 were APPROVED; implementation was not to begin before independent plan review. See `docs/implementation/milestones/M2.6-current-state-audit.md`, `M2.6-owner-decisions.md`, and `M2.6-idempotency-concurrency-plan.md`.
>
> **M2.6 plan-review remediation (2026-07-28, Historical pre-merge plan context):** independent implementation plan review of `70d965b9` returned BLOCKER 2 / HIGH 2 / MEDIUM 3 / LOW 0. All seven findings were resolved in the remediation commit (OD-5 key normalization + rawHeaders parser freeze; OD-13 typed envelope V1 without `unknown`; OD-18 exact Migration 010 DDL + trigger + exports; OD-19 request-hash verifiability boundary; P1 test-scope audit covering `Identity.test.ts`, `m2-4-task-run-schema.test.ts`, `m2-5-workflow-snapshot-stage-schema.test.ts`; `findVerifiedByScope` repository contract; TaskRunService/Route result contract freeze; revised 43-case test matrix). Historical pre-merge status: M2.6 IMPLEMENTATION PLAN REVIEW REMEDIATION COMPLETE — PENDING FINAL PLAN REVIEW; M2.6 P1 NOT authorized.

> **M2.6 final verification closure (2026-07-29, Historical pre-merge status):** P1–P4 implementation completed on `runtime/m2-6-idempotency-concurrency`; P5 security and compatibility verification passed locally. P4 head was `8559d434fc60e176e9cfa32e6f81257be8b69e31`; P5 report was `docs/implementation/milestones/M2.6-idempotency-concurrency-report.md`; Migration 010 was implemented and locally verified; no backfill or destructive migration was performed. Historical pre-merge status: final independent review remained pending; no PR or merge was claimed; M2.7 and M2.8 were not started.

> **M2.6 final review closure (2026-07-29, Historical pre-merge status):** the final independent review of `cefff26b7fcbb4cd8915e9b5a2a780b847159b3b` PASSED with BLOCKER 0 / HIGH 0 / MEDIUM 0 / LOW 0; P1–P5 final reviews all passed; M2.6 IMPLEMENTATION FINAL REVIEW PASSED. Migration 010 was implemented and locally verified; historical pre-merge status was that M2.6 had not been merged and Migration 010 had not entered Main (`39eb1d5a` unchanged); no PR had been created; M2.7 and M2.8 were not started. Migration 001–009 history was unchanged.

> **M2.6 post-merge closeout (2026-07-29):** PR #8 entered Main through Merge Commit `6727add8303b7d0ab659a427bfdd8299a98e5702`, with reviewed base `39eb1d5ae2238f9f65fe7475fa3271a93a946acd` and reviewed head `5416729fbc7b23ba7c6df0856ffefb7c024a5c80`. Migration 010 is IMPLEMENTED, VERIFIED AND MERGED; no backfill or destructive migration was performed; Migration 001–009 history is unchanged; pre-010 additive upgrade evidence is retained; rollback remains a code-level integration revert with records inert; Remote CI is NONE / NOT CLAIMED; M2.7 and M2.8 are not started.

---

> **M2.7 Historical Draft Notice (2026-07-29):** the following M2.7 section is retained for historical traceability only. It is a stale candidate and is superseded for current planning by the M2.7 audit, Owner Decision register, and plan draft named in the Override below.
...
> **M2.7 Current-State Audit Override (2026-07-29):** the M2.7 section immediately above is a Historical / Stale Candidate Draft. It is not implementation authority and must not be used to begin P1. Current facts are recorded in `docs/implementation/milestones/M2.7-current-state-audit.md`; Owner questions are in `docs/implementation/milestones/M2.7-owner-decisions.md`; the implementation plan draft is `docs/implementation/milestones/M2.7-v1-compatibility-migration-plan.md`. The current branch is `runtime/m2-7-v1-compatibility-migration` from Main `30b3b20c9c4239fdfc9d423497b04c94e9468dde`. M2.7 status is CURRENT-STATE AUDIT AND PLANNING IN PROGRESS — IMPLEMENTATION NOT AUTHORIZED. No Migration 011, JSON deletion, JSON fallback, historical Run synthesis, or Legacy API removal is assumed. M2.8 remains NOT STARTED.
...
## M2.4 Project-Root Ownership Remediation Note (2026-07-25 — HISTORICAL 时点记录，当前状态以本文档顶部 Status 与状态表为准)
