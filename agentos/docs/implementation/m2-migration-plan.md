...
| M2.6 | IMPLEMENTATION PLAN REVIEW REMEDIATION COMPLETE — PENDING FINAL PLAN REVIEW; implementation not started | Branch `runtime/m2-6-idempotency-concurrency`; OD-1–OD-20 APPROVED 2026-07-28 (planning review BLOCKER 0 / HIGH 0 / MEDIUM 4 / LOW 1, all resolved); independent implementation plan review of `70d965b9` returned BLOCKER 2 / HIGH 2 / MEDIUM 3 / LOW 0, all seven findings resolved by the 2026-07-28 remediation (request-hash verifiability boundary, exact Migration 010 DDL, typed envelope V1 without `unknown`, rawHeaders key parser, `findVerifiedByScope` repository contract, TaskRunService/Route result contract freeze, P1 test-scope audit); see `docs/implementation/milestones/M2.6-current-state-audit.md`, `M2.6-owner-decisions.md`, `M2.6-idempotency-concurrency-plan.md`; M2.6 P1 NOT authorized; no production code, test, migration, or shared type change authorized; M2.7/M2.8 not started |
...
> **M2.6 override (2026-07-28):** the detailed M2.6 draft above is pre-M2.5 history and is **not** implementation authorization. Current facts: the migration registry is TS-based 001–009 (M2.5 consumed 007–009), so `v008-idempotency-records.sql` is stale and the next migration candidate is **010** (exact schema frozen in OD-18); `routes/tasks.ts`/`routes/runs.ts` are Legacy/Conversation surfaces and do not constitute implementation authority — the canonical Task-domain API is the workspace-scoped v2 routes; optimistic version infrastructure is partially implemented (Repository version guards); Approval is currently an in-memory Registry and is deferred by OD-17. Owner Decisions OD-1–OD-20 are APPROVED; implementation must not begin before independent plan review. See `docs/implementation/milestones/M2.6-current-state-audit.md`, `M2.6-owner-decisions.md`, and `M2.6-idempotency-concurrency-plan.md`.
>
