...
| 4 Stage Transition | Initial run_stages rows are persisted with status limited to pending; Migration 009 already provides version, but no Task-domain stage transition engine or version-checked transition exists. | Stage definitions, ordering, snapshot persistence, and the existing version field exist. | M3 Stage lifecycle must be persisted, ordered, legal, version-safe, and reflected in Run Events. | Preserve the existing version column; extend only Stage status vocabulary, failure fields, started/completed timestamps, and version-checked transition logic through the reviewed Migration 012 scope. | No Worktree, Policy, Approval, or provider-specific stage behavior; do not add a duplicate version column. | apps/server/src/migrations/migrations/009-run-stages.ts; apps/server/src/store/RunRepository.ts; Runtime-Specification/02-Runtime-Lifecycle.md §§6, 57 |
...
| 8 Outbox table, repository, publisher | Registry 001–011 has no outbox_messages table, repository, publisher, retry, or independent dead_letters or reviewed equivalent. | Migration and EventBus infrastructure shows transaction and publication patterns in other domains. | Outbox identity/payload fields cannot be rewritten; delivery fields may change only through a guarded state machine with concurrency protection; publication is at-least-once and deduplicated by durable identity. | Plan immutable fields id/event reference/topic/aggregate/payload/created_at, controlled delivery fields status/attempts/available_at/published_at/last_error and optional lease/fencing/version, conditional updates, and dead letters. | Do not prohibit all UPDATE on outbox_messages; no external broker or paid infrastructure. | apps/server/src/migrations/default-registry.ts; Runtime-Specification/10-Data-Model.md §§78, 111 |
...
| 14 Operation Resource | No durable Operation table, repository, or operation route is present. | Start-like work can be represented by a Run row, but there is no separate command lifecycle. | M3 Operation tracks only Task-domain Run commands and persists workspaceId, aggregateType=run, aggregateId/runId, immutable unique correlationId, exact status queued/running/waiting_approval/paused/completed/failed/cancelled, result, ApiProblem, timestamps, and version. | Add durable Operation schema and GET /api/operations/:operationId, GET /api/operations/:operationId/events, POST /api/operations/:operationId/cancel. The Events handler must authorize the Operation, use its runId and correlationId, query runtime_events in sequence order, and must not create operation_events. | Non-Run Operations are Post-M3 design; do not use Operation to authorize cutover or production work. | apps/server/src/routes/v2Runs.ts; Runtime-Specification/11-API-Specification.md §§31, 32 |
...
5. A durable Operation resource with operations.run_id or equivalent aggregate reference.
6. Operation lifecycle, result, ApiProblem, aggregate reference, timestamps, version, workspaceId, aggregateType=run, aggregateId/runId, and correlationId fields.
7. correlationId uniqueness and query constraints; correlationId is immutable.
8. A rebuilt or extended run_stages schema. Migration 009 already provides version; M3 adds Stage status vocabulary, failure_code/failure_message, started_at/completed_at, and version-checked updates without adding another version column.
9. idempotency_records.operation values for run.start, run.retry, and the finally approved M3 command set.
10. An explicit Run recovery representation: either runs.recovery_required or a separate Recovery Record. P6 must not use a state absent from the schema.
11. A Task-domain sequence allocator that uses runs.next_event_sequence.
12. Append-only Event and Outbox constraints under concurrency, including runtime_events(run_id, correlation_id, sequence) query indexing and controlled delivery updates.
13. A durable Queue decision. M3 uses runs(status=queued) as the persistent Queue Record; scheduler_jobs is not added unless later evidence proves the invariant cannot be met.
...
### 6.3 Operation Event association

M3 Operation is only a tracker for Task-domain Run commands. Each Operation stores workspaceId, aggregateType=run, aggregateId/runId, and a correlationId that is unique and immutable. Non-Run Operations are deferred to a Post-M3 design.

GET /api/operations/:operationId/events must:

1. Read and authorize the Operation.
2. Use the Operation runId and correlationId.
3. Query runtime_events.
4. Return matching events in ascending sequence order.

M3 does not add an independent operation_events Event Store. The correlationId is the Operation query/association key, not a second event authority.

### 6.4 Outbox mutability boundary

The future outbox_messages contract distinguishes immutable identity/payload fields from controlled delivery fields.
...
- published_at.
- last_error.
- lease, fencing, or version fields if the final DDL adopts them.

The M3 contract does not prohibit all UPDATE on outbox_messages. Delivery updates must use a state machine, conditional UPDATE, and concurrency protection. Payload and identity fields cannot be rewritten. Durable identity provides deduplication. Dead Letter remains an independent table or a separately reviewed equivalent.

### 6.5 Race-free SSE handoff
