...
Status: PREPLANNING ONLY — IMPLEMENTATION REQUIRES PHASE-BY-PHASE AUTHORIZATION
...
- compatibility and integrated verification.

Potentially included only after the initial Kimi gate and separately authorized:

- Codex adapter as the second proof of the generic seam.

Deferred follow-up:

- OpenCode adapter until a real executable/protocol and dedicated adapter
...
M4-P0 contract/current-state closure
  ...
  -> M4-P7 integrated verification/closeout

Optional, separately authorized after P4/P5:
  Codex adapter proof

Deferred:
  OpenCode adapter; M5-owned Git/Worktree/Artifact process migration; M7 hardening
```
...
separate authorization route; exactly one Kimi slice.
...
fallback; spec conflict changes product semantics.
...
### M4-P7 — Integrated verification and closeout

**Goal:** prove E01–E10 and phase contracts on one immutable head, document
remaining deferrals, and stop.
...
**Excluded:** new behavior, Codex/OpenCode unless separately authorized,
production cutover, Ready/merge by default, Legacy retirement.

**Allowed files/packages:** tests/fixtures and closeout docs expressly owned by
...
Kimi vertical slice passes; no unauthorized scope.
...
**Independent review gate:** final cross-domain review. Acceptance closes M4
implementation evidence only and does not authorize cutover/Ready/merge.

**Next phase authorization:** any Codex adapter, OpenCode adapter, M5, M7,
cutover, Ready or merge requires a new explicit authorization.
...
- **Codex:** same M4 only as a later, separately authorized proof after the Kimi
  slice and Process cancellation gates pass. Current dedicated adapter and real
  historical evidence reduce risk, but it must not share the initial phase.
- **OpenCode:** deferred follow-up. Current main lacks a dedicated registered
  adapter and current machine lacks the executable. Model discovery/usage delta
  are not an Adapter contract.
- **Custom CLI:** only the minimal fallback contract needed to preserve
  compatibility; no broad product surface or silent fallback.
...
| tests/fixtures/closeout evidence | P7 | production remediation requires explicit re-entry |
...
| Real gate | installed Kimi direct validation and bounded task through AgentOS; current version/path recorded; deterministic fake gate remains the required CI baseline |
...
M4 exit requires all ten. No subset authorizes production cutover.
...
- P7: complete E01–E10 evidence and unauthorized-scope audit.
...
- P7 changes tests/evidence/docs; findings send work back to an authorized phase.
...
- phase broadens into Codex/OpenCode/M5/M7/UI/cutover without authorization;
...
2. One accepted Run/Stage attempt creates exactly one Kimi Process through the
   single authority chain.
3. AgentOS Process, PID, Run, Stage, Provider Configuration, Provider Session
   (if present) and old Execution identities are unambiguous.
4. durable Process state/raw output/Process facts satisfy schema, security and
   replay contracts.
5. active cancel kills/verifies the owned tree on Windows and POSIX contracts.
6. disconnect never cancels; explicit Cancel does.
7. restart classification never guesses success and preserves M3 uncertainty.
8. Provider auth/config/version/output failures have stable semantics.
9. M3 lifecycle/Event/Outbox/Operation/idempotency/compatibility tests pass.
10. remaining direct subprocesses are inventoried and assigned to M4/M5/other
    ownership; none is silently declared compliant.
11. no Legacy retirement, Web switch, cutover, M7 hardening or additional
    Provider scope was smuggled in.
12. final independent review has `BLOCKER = 0`, `HIGH = 0`.

Codex/OpenCode completion is not required for the initial Kimi vertical-slice
gate. If Roadmap M4 closure later requires Codex as the second adapter proof,
that work receives its own phase authorization and review. OpenCode remains
...
```text
M4 PREPLANNING:
COMPLETE WHEN THIS DOCS PACKAGE IS ACCEPTED
...
The next action after this docs package is independent M4 preplanning review.
