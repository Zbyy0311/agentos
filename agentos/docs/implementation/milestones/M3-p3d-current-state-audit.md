...
> 审计日期：2026-08-07。
>
> 合同闭合补救：M3-TD-31 与 M3-TD-32 已由 Owner 批准并写入合同；
> 生产实现仍未授权。本文中的实现状态继续以当前代码为准。
...
| 范围项 | 状态 | 审计结论 |
| --- | --- | --- |
| P3D-1 Operation 只读面 | PREPLANNING COMPLETE / NOT AUTHORIZED | 只规划 GET /api/operations/:operationId 与 GET /api/operations/:operationId/events；生产实现未授权。 |
| P3D-2 原子 Cancel 核心 | CONTRACT READY / NOT AUTHORIZED | M3-TD-31/32 已闭合合同；本轮不实现 POST /api/operations/:operationId/cancel。 |
| P3D-3 竞态与失败闭环 | PREPLANNING COMPLETE / NOT AUTHORIZED | 只规划竞态/回滚证据；依赖 P3D-2，且本轮不实现。 |
...
以下是保留的 P3D-0 baseline evidence。本次合同闭合为 docs-only，未重新运行生产测试，不把保留证据描述成本轮新 PASS。
...
| M3-TD-31 HTTP request/replay contract | CONTRACT CLOSED / IMPLEMENTATION MISSING | URL 只接 operationId、query 为空、body 精确为 required positive-safe-integer expectedVersion；不接受 ETag/If-Match 或 client lifecycle metadata。already-cancelled stale-version no-op 与其他 conflict precedence 已冻结。 |
| M3-TD-32 guarded/approval-aware contract | CONTRACT CLOSED / IMPLEMENTATION MISSING | Option C 已批准：专用 Operation guarded cancel + approval-aware Lifecycle seam；普通 Operation transition table 不扩展。 |
| Cancel 的四类 Operation 状态 | CONTRACT CLOSED / IMPLEMENTATION GAP | M3-TD-27/32 冻结 queued/running/waiting_approval/paused；当前 OperationService.ALLOWED_TRANSITIONS 对 waiting_approval、paused 设置空出边，且当前没有专用 guarded seam。 |
| Cancel request body/header 的精确形状 | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 与 11-API-Specification.md §33.2 已冻结 exact body、空 query、locator-first parser order、无 ETag transport 与响应优先级；生产 route 尚不存在。 |
| Cancel trusted server metadata | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 冻结 requestedBy=operation_api、terminatedProcessIds=[]、worktreePreserved=true、reason absent；HTTP 不接受这些字段，且不得复制 v2_api/false。 |
| Cancel Event payload | IMPLEMENTED FOUNDATION / P3D NOT IMPLEMENTED | 03-Event-Model.md 与 LifecycleTransactionService.CancelRunInput 已有 payload shape；M3-TD-31/32 冻结 P3D trusted values 与 waiting-approval Event 顺序，但 P3D composition 尚未实现。 |
...
| P3D guarded cancel | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-32 批准 Option C，但 OperationService 仍没有 cancelWithinTransaction 或等价 caller-owned cross-aggregate orchestration；普通 transition table 必须保持不变。 |
...
| Direct Run cancellation core | IMPLEMENTED FOUNDATION | LifecycleTransactionService.cancelRunWithinTransaction 是 caller-owned 且覆盖 queued/starting/running/paused；M3-TD-32 允许 future approval-aware seam 对这些状态复用它，且不得改变 v2 行为。 |
| Approval-aware Operation cancellation | CONTRACT CLOSED / IMPLEMENTATION GAP | M3-TD-32 冻结 exactly-one unresolved Approval discovery 与 approval.resolved -> stage.cancelled* -> run.cancelled，但当前 LifecycleTransactionService 尚无该专用 seam。现有 resolveApprovalToCancellation 需要外部已知 approval identity，不能直接替代 discovery。 |
...
| Store composition | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-32 允许 P3D-2 narrow 修改 SqliteStore；当前仍分离构造 LifecycleTransactionService 与裸 database OperationService，尚未共享 P3D outer transaction ownership。 |
...
| cancellable statuses | CONTRACT CLOSED / IMPLEMENTATION GAP | M3-TD-32 批准 Option C 与四状态专用 guard；普通 ALLOWED_TRANSITIONS 不得扩展，当前专用 guard 尚不存在。 |
| terminal behavior | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 冻结：matching-version completed/failed -> 409 OPERATION_NOT_CANCELLABLE；当前无 route/service mapping。 |
| already cancelled behavior | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 冻结：HTTP 200 current ApiOperation、zero side effects，并优先于 stale expectedVersion；当前无专用入口与测试。 |
...
| waiting_approval Run | IMPLEMENTATION GAP / CONTRACT CLOSED | M3-TD-32 已冻结 approval-aware discovery/sequencing seam；当前代码未实现，普通 cancelRunWithinTransaction 仍正确拒绝 waiting_approval。 |
| request shape | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 冻结 exact expectedVersion body、empty query、no ETag、no client metadata 与 operation_api/[]/true/undefined 服务端 metadata；route 尚未实现。 |

## 5. 原子组合、竞态与失败注入准备度

### 5.1 目标交易顺序

M3-TD-31/32 的目标是 locator-first HTTP order 加一个 caller-owned transaction。未来实现不得拆成两个独立 HTTP/service transaction：

1. Operation Router 在全局 express.json() 前 mount；opaque operationId locator 先解析并授权 workspace。
2. Cancel 的 route-scoped JSON parser 与 exact empty-query/body validator 在 locator 后运行。
3. 进入唯一 BEGIN IMMEDIATE，并 workspace-scoped re-read Operation。
4. 校验 Operation 的 aggregateType=run、aggregateId=runId、workspace 与 correlation binding。
5. 若 Operation 已为 cancelled，直接 HTTP 200 返回当前 Operation；即使 expectedVersion stale，也禁止 lifecycle/Event/Outbox 副作用。
6. 其他状态比较 Operation expectedVersion；stale 返回 VERSION_CONFLICT。
7. matching-version completed/failed 返回 OPERATION_NOT_CANCELLABLE；四类可取消状态进入专用 guarded update。
8. 条件更新 Operation 为 cancelled，再由 approval-aware lifecycle seam 重读 bound Run，并使用 fresh persisted Run.version。
9. 非 approval Run 可复用现有 cancel core；waiting_approval 必须发现 exactly one unresolved Approval 并保持 approval.resolved -> stage.cancelled* -> run.cancelled。
10. 任一状态、Event、Outbox、version、sequence 或 commit 失败均回滚全部；成功才返回 ApiOperation。
...
| 输入/规则 | 状态 | 当前证据与实现边界 |
| --- | --- | --- |
| workspaceId、Operation expectedVersion、correlation context | CONTRACT CLOSED / IMPLEMENTATION MISSING | HTTP 只携带 Operation expectedVersion；workspace/run/correlation 来自 locator 与绑定 Operation，不接受客户端覆盖。 |
| expectedRunVersion | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-32 冻结为 outer BEGIN IMMEDIATE 内 fresh re-read 的 persisted Run.version；HTTP 不接受 expectedRunVersion。 |
| requestedBy | CONTRACT CLOSED / IMPLEMENTATION MISSING | 服务端固定为 operation_api；HTTP 不接受该字段。 |
| terminatedProcessIds | CONTRACT CLOSED / IMPLEMENTATION MISSING | 服务端固定为 []，表示 P3D 不声明终止 M4 OS Process；不得照抄为客户端字段。 |
| worktreePreserved | CONTRACT CLOSED / IMPLEMENTATION MISSING | 服务端固定为 true，表示不删除、重置、清理或修改 Worktree。 |
| reason | CONTRACT CLOSED / IMPLEMENTATION MISSING | P3D canonical Cancel 固定 absent/undefined；HTTP 不接受 reason。 |
| waiting approval | IMPLEMENTATION GAP / CONTRACT CLOSED | M3-TD-32 要求 exactly-one unresolved Approval discovery 与冻结 Event 顺序；当前专用 seam 不存在。 |
...
| expectedVersion race | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 只接受 body.expectedVersion；stale non-cancelled -> 409 VERSION_CONFLICT，already-cancelled no-op 优先；不接受 ETag/If-Match。 |
| Operation/Run/Approval binding inconsistency | CONTRACT CLOSED / IMPLEMENTATION MISSING | fail closed、outer rollback，并在 route sanitize 为 500 INTERNAL_ERROR；本轮不新增 public inconsistency code。 |
| lifecycle/request validation | CONTRACT CLOSED / IMPLEMENTATION MISSING | exact query/body 失败为 VALIDATION_FAILED；P3D route 必须屏蔽 SQLite/SQL/path/Event internals/stack，不得落入 raw message fallback。 |
...
| locator-before-parser | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 冻结 Operation Router mount 在 global express.json() 前；locator 先于 Cancel route-scoped parser 与 exact validator。当前 Operation Router 尚不存在。 |
...
| OperationService cross-aggregate cancel orchestration | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31/32 冻结 Option C 与 outer transaction；当前构造函数无 Lifecycle dependency。 | 尚不能原子更新 Operation/Run/Stage/Event/Outbox。 | P3D-2 future required scope。 |
| waiting_approval cancellation seam | IMPLEMENTATION GAP / CONTRACT CLOSED | M3-TD-32 已冻结 exactly-one unresolved Approval discovery 与 Event 顺序；生产代码尚无 seam。 | 普通 cancelRunWithinTransaction 不能处理 waiting_approval。 | LifecycleTransactionService.ts 在 P3D-2 为 owner-approved narrow REQUIRED；P3D-1 仍 FORBIDDEN。 |
| Cancel request exact shape | CONTRACT CLOSED / IMPLEMENTATION MISSING | M3-TD-31 冻结 exact expectedVersion body、empty query、no ETag/no client metadata、response precedence。 | route 尚不存在。 | 不再是 owner-decision blocker；等待 P3D-2 独立实现授权。 |
| guarded cancel status path | CONTRACT CLOSED / IMPLEMENTATION MISSING | Option A REJECTED；Option B REJECTED AS INCOMPLETE；Option C OWNER APPROVED。 | 当前没有专用 guarded Operation seam。 | 普通 ALLOWED_TRANSITIONS 保持不变。 |
...
- Contract closure：COMPLETE。M3-TD-31/32 已关闭 request、response precedence、metadata、Option 与 waiting-approval lifecycle direction；不再存在未决 P3D Owner Decision。
- Production authorization：NOT AUTHORIZED。合同就绪不等于实现获准；P3D-1/P3D-2/P3D-3 都必须等待独立远程技术复审和单独授权。
- waiting_approval：IMPLEMENTATION GAP / CONTRACT CLOSED。Future P3D-2 必须实现 narrow approval-aware seam；本轮不修改 LifecycleTransactionService。
- Request/Router：CONTRACT CLOSED / IMPLEMENTATION MISSING。Future Router 必须在 global express.json() 前 mount，locator 先于 route-scoped parser。
- Implementation composition gap：MISSING。Future P3D-2 可 narrow 修改 SqliteStore 共享同一 database handle/transaction ownership；不得提前进入 P3D-1。
...
| 决策 | 状态 | 理由 |
| --- | --- | --- |
| Option A：扩大普通 Operation transition table | REJECTED | 会扩大所有普通 transition caller 的能力，削弱 Cancel 专用 guard，并可能影响 Engine/adjacent consumers。 |
| Option B：专用 Operation guard + 普通 Run cancel seam | REJECTED AS INCOMPLETE | 普通 cancelRunWithinTransaction 明确拒绝 waiting_approval；直接扩大它会绕过 Approval ordering。 |
| Option C：专用 guarded Operation cancel + approval-aware lifecycle seam | OWNER APPROVED / NOT IMPLEMENTED | M3-TD-32 已冻结；普通 ALLOWED_TRANSITIONS 不变，生产代码尚未实现。 |
| Cancel HTTP request/replay contract | OWNER APPROVED / NOT IMPLEMENTED | M3-TD-31 冻结 exact body/query、locator/parser、response precedence、trusted metadata、idempotency 与 stable errors。 |
| Operation Router/parser order | OWNER APPROVED / NOT IMPLEMENTED | Router 在 global express.json() 前；locator -> workspace authorization -> route-scoped parser -> exact validation -> service。 |
...
| LifecycleTransactionService phase scope | OWNER APPROVED / NOT IMPLEMENTED | P3D-1 FORBIDDEN；P3D-2 narrow REQUIRED，仅增加 approval-aware Operation cancellation seam。 |
| SqliteStore P3D-2 composition | OWNER APPROVED / NOT IMPLEMENTED | 只允许同一 database handle/transaction ownership wiring；不新增 service/database/singleton/worker。 |
| 不创建 Migration 014 | NOT REQUIRED | schema sufficiency 已闭合。 |

## 9. 推荐 allowlist（未来生产实现，不是本轮授权）

### 9.1 P3D-1 REQUIRED（DEPENDENCY READY / NOT AUTHORIZED）

1. apps/server/src/routes/operations.ts
2. apps/server/src/routes/operations.test.ts
3. apps/server/src/services/OperationService.ts
4. apps/server/src/services/OperationService.test.ts
5. apps/server/src/store/OperationRepository.ts
6. apps/server/src/store/OperationRepository.test.ts
7. apps/server/src/index.ts

P3D-1 只实现两个 GET endpoint，不实现 Cancel。现有 SqliteStore.operationService accessor 足够，因此 SqliteStore.ts 在 P3D-1 为 FORBIDDEN；若未来独立 composition evidence 证明必须暴露 Router accessor，才可重新分类为 CONDITIONAL。

### 9.2 P3D-2 REQUIRED（CONTRACT READY / NOT AUTHORIZED）

1. apps/server/src/routes/operations.ts
2. apps/server/src/routes/operations.test.ts
3. apps/server/src/services/OperationService.ts
4. apps/server/src/services/OperationService.test.ts
5. apps/server/src/store/SqliteStore.ts
6. apps/server/src/services/LifecycleTransactionService.ts
7. apps/server/src/services/m3-p2c2b-composite-lifecycle.test.ts（REQUIRED：Approval discovery/resolution、ordered cancellation、rollback、concurrency）

apps/server/src/services/m3-p2c2a-lifecycle-transaction.test.ts 为 REGRESSION ONLY；P3D-2 不改变 single-transition semantics。OperationRepository.ts/OperationRepository.test.ts 只有在 P3D-1 后仍有 cancel-specific conditional-update evidence 时才是 CONDITIONAL，否则不重复扩大。

### 9.3 P3D-3 boundary（NOT AUTHORIZED）

P3D-3 只证明 Claim vs Cancel、Start Completion vs Cancel、Startup Failure vs Cancel、duplicate/already-cancelled/terminal Cancel、two SQLite connections、failure injection 与 rollback closure。它不得新增 product behavior，不得修改 RunEngine production code。

### 9.4 全阶段 FORBIDDEN

Shared/Registry/Migration 014、Web、SSE/replay/OpenAPI completion、TaskRunService/v2 behavior、ProcessManager/provider/CLI/worktree runtime、非 Run Operation、policy、production cutover，以及删除/重命名 run.cancel 或增加 operation.cancel idempotency，均保持 FORBIDDEN。

## 10. P3D-0 结论

| 结论 | 状态 | 说明 |
| --- | --- | --- |
| P3D-0 PREPLANNING | COMPLETE | 当前状态、代码 seam、schema、竞态、失败注入、gap 与 phase allowlist 已记录。 |
| P3D CONTRACT CLOSURE | COMPLETE | M3-TD-31/32 已 Owner approved/documented；合同闭合不等于生产实现。 |
| M3-TD-31 | OWNER APPROVED / NOT IMPLEMENTED | HTTP request/replay contract 已冻结。 |
| M3-TD-32 | OWNER APPROVED / NOT IMPLEMENTED | Option C guarded + approval-aware lifecycle contract 已冻结。 |
| P3D-1 PRODUCTION IMPLEMENTATION | NOT AUTHORIZED | DEPENDENCY READY；仅允许未来独立授权的两个 GET endpoint。 |
| P3D-2 PRODUCTION IMPLEMENTATION | NOT AUTHORIZED | CONTRACT READY；依赖 P3D-1 acceptance。 |
| P3D-3 PRODUCTION IMPLEMENTATION | NOT AUTHORIZED | 依赖 P3D-2，只做 race/failure closure。 |
| P3E | NOT ENTERED / NOT AUTHORIZED | 未进入。 |
| Migration 014 | NOT REQUIRED / NOT AUTHORIZED / ABSENT | 当前 schema 足够，未创建。 |
| Production cutover | NOT AUTHORIZED / NOT STARTED | 未执行。 |
