...
(P3A–P3E). Nothing here is authorized work.

| # | Gap | Current evidence | Required target | Category | Depends on | Test evidence | Stop condition | Rollback boundary |
|---|-----|------------------|-----------------|----------|------------|---------------|----------------|-------------------|
| 1 | Persistent queue ownership | `runs(status='queued')` exists; no owner | Exactly one component (Run Engine) may claim queued runs | P3B | P3A | Ownership unit test: only the engine mutates claimed runs | Any second writer to queued runs appears | Revert engine as one package; queue rows untouched |
| 2 | Run claim/dequeue | No claim path | Atomic claim of a queued run (status/version guarded) inside the transaction core | P3B | #1 | Two competing claims, exactly one succeeds | Claim requires a new table or bypasses the transaction core | Revert claim path; no data reset |
| 3 | Run Engine | Absent (3.3) | Tick-driven engine advancing claimed runs via `LifecycleTransactionService` only | P3B | #2 | Engine unit tests with injected transaction core | Engine calls repositories directly, bypassing events/outbox | Revert engine package |
| 4 | Deterministic Workflow Executor | Absent (3.5) | Executor reading the run snapshot V2 stage graph (`dependsOn`) deterministically; mock stage runner, no provider runtime | P3B | #3 | Snapshot-graph traversal tests; deterministic ordering proof | Executor needs ProcessManager/ProviderAdapter/CLI | Revert executor package |
| 5 | Stage orchestration | Absent (3.6) | Stage lifecycle driven through `transitionStage`; `skipped` propagation on failure/cancel per spec | P3B | #4 | Stage transition and skip-propagation tests | Stage writes bypass Event/Outbox | Revert orchestration; stage rows preserved |
...
| 7 | HTTP 202 Start | Absent (3.8) | Start route returns 202 + Operation; run stays queued until engine claims | P3C | #6, P3B | Route contract test: 202 shape, Operation body | Synchronous start execution in the route handler | Revert route; runs/operations untouched |
| 8 | Duplicate Start | `run.start` idempotency schema-accepted, no consumer (3.9) | Same idempotency key replays the original Operation; different key on an already-started run is rejected per contract | P3C | #7 | Same-key replay test; different-key rejection test | Duplicate start mutates the run twice | Revert start idempotency wiring |
| 9 | Start failure rollback | Failure-injection harness exists (3.15) | Failure at any event position in start leaves zero partial state and a failed Operation with `error_json` | P3C | #7 | Injection at each event call position; post-failure integrity checks | Any partial commit observed | Revert start path; preserve durable evidence, no data reset |
...
| 11 | Retry child Run | Lineage computation exists; no caller (3.11) | Retry creates a child run (new id, parent lineage), never resets the old run; `run.retry` operation recorded | P3C | #6 | Lineage test: root id, parent id, old run untouched | Retry mutates or resets the parent run | Revert retry path |
...
| 15 | Idempotency coverage | 6 consumers wired; `run.start`/`run.retry` unwired (3.9) | All P3 mutating routes use `IdempotencyService` consistently | P3C | #7, #11 | Per-route idempotency tests | A P3 route skips the idempotency service | Revert wiring |
| 16 | `recovery_required` interaction | Column and setting paths exist from P2 | P3 transitions leave the flag semantics unchanged; startup recovery continues to own it | P3B/P3C | P2 core | Regression: flag set/clear paths unchanged | P3 code writes the flag directly | Revert offending path |
...
| 19 | correlationId generation for non-create operations | Mechanism exists; rule undecided (3.14) | Owner-approved deterministic rule for `run.start`/`run.cancel`/`run.retry` correlation ids, UNIQUE per operation | P3A (decision), P3C (use) | OD-P3-01 | Uniqueness and association tests per decided rule | Implemented without Owner Decision | Revert generation wiring |
...
Conclusion: Migration 012 (with 001–011 and 013) is sufficient for the entire
P3 scope. No Migration 014 is required for P3 as scoped.

SCHEMA BLOCKER — none. (Had any insufficiency been found, this audit would
have recorded `SCHEMA BLOCKER — OWNER DECISION REQUIRED` and stopped.)

## 6. Owner Decision Candidates

The following are decision candidates surfaced by this audit. They are NOT
approved decisions. P3 implementation must not resolve them silently.
...
- P3B — Run Engine and Deterministic Workflow Executor
...
of Start/Cancel/Retry routes (P3C/P3D), and the engine (P3B) is the
dependency of observable async behavior (P3C).
