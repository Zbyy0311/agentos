...
- 批次收据：`docs/implementation/lite-closeout/evidence/s8-runtime-verify-batches-20260914/verification-batches-v16.json`
- 收据绑定基线：`d05fe42881ac3f53cf4009255168268e3befa8d6`
- 批次汇总：`{"batches":51,"passed":48,"failed":3,"requirementsProvable":0,"requirementsTotal":225}`
...
| `not-in-batch / no-assertion-parsed` | 1 |
| `passed / no-clause-match` | 47 |
...
| `LITE-00-003` | `apps/server/src/services/RuntimeInspector.test.ts` | `passed` | 12 | INSP-12 Run, Stage, Provider, Process and duration stay distinct | candidate-supported |
| `LITE-00-004` | `apps/server/src/routes/canonicalRunStream.test.ts` | `passed` | 16 | P5C-R06 browser disconnect is subscription-only: Run state untouched and lifecycle continu | candidate-supported |
...
| `LITE-00-006` | `apps/server/src/services/run-engine/RunEngineProviderDispatcher.test.ts` | `not-clean` | 33 | P5E composes Dispatcher cancellation, owned Process cleanup, and LTS handoff exactly once | candidate-supported |
| `LITE-00-007` | `apps/server/src/services/TaskRunRecoveryService.test.ts` | `passed` | 67 | P6M2b running + mismatch -> no recovery, recovery failure (uncertainty) recorded | candidate-supported |
...
| `LITE-00-009` | `apps/server/src/services/run-engine/RunEngineProviderDispatcher.test.ts` | `not-clean` | 33 | LITE-08-006/007: reject is terminal, replay-safe, and a changed launch plan cannot execute | candidate-supported |
...
| `LITE-01-004` | `apps/server/src/services/TaskRunService.test.ts` | `passed` | 92 | S13 task.accept replay is evaluated before the acceptance and transition guards | candidate-supported |
| `LITE-01-005` | `apps/server/src/services/TaskRunService.test.ts` | `passed` | 92 | P3C1-RY-S01 Retry acceptance creates a queued Child and completed v3 Operation | candidate-supported |
| `LITE-01-006` | `apps/server/src/services/RuntimeInspector.test.ts` | `passed` | 12 | INSP-12 Run, Stage, Provider, Process and duration stay distinct | candidate-supported |
...
| `LITE-02-012` | `apps/server/src/services/TaskRunService.test.ts` | `passed` | 92 | P3C1-RY-S01 Retry acceptance creates a queued Child and completed v3 Operation | candidate-supported |
| `LITE-02-013` | `apps/server/src/services/run-engine/RunEngineProviderDispatcher.test.ts` | `not-clean` | 33 | LITE-08-007: expiry after the first gate but before spawn blocks the Process side effect | candidate-supported |
...
| `LITE-05-001` | `packages/process-runtime/src/durable-coordinator.test.ts` | `passed` | 34 | — | insufficient-evidence |
...
| `LITE-05-011` | `apps/server/src/taskRecovery.test.ts` | `passed` | 29 | P6M2b-composition E: no classifier/verifier call occurs inside the recovery transaction | candidate-supported |
...
| `LITE-07-002` | `apps/server/src/store/MemoryCandidateRepository.test.ts` | `passed` | 24 | MF2R-03 automatic candidate without source rejects | candidate-supported |
...
| `LITE-07-006` | `apps/server/src/routes/memoryRuntime.test.ts` | `passed` | 16 | MF-5 retrieve: ranked results with reasons, filters, limit, and degraded flag | candidate-supported |
...
| `LITE-08-001` | `apps/server/src/services/run-engine/RunEngineProviderDispatcher.test.ts` | `not-clean` | 33 | MF-4 integration: a blocked injection prevents provider execution | candidate-supported |
| `LITE-08-002` | `apps/server/src/services/run-engine/RunEngineProviderDispatcher.test.ts` | `not-clean` | 33 | LITE-08-007: expiry after the first gate but before spawn blocks the Process side effect | candidate-supported |
...
| `LITE-08-008` | `apps/server/src/services/run-engine/RunEngineProviderDispatcher.test.ts` | `not-clean` | 33 | LITE-08-006/007: reject is terminal, replay-safe, and a changed launch plan cannot execute | candidate-supported |
...
| `LITE-09-002` | `apps/server/src/services/ConversationBridgeService.test.ts` | `passed` | 14 | CR4B-02 create-task is idempotent across retries and starts nothing | candidate-supported |
...
| `LITE-09-012` | `apps/server/src/services/BoundedGroupService.test.ts` | `passed` | 17 | CR5S-10 hops beyond the configured limit terminate the interaction | candidate-supported |
...
| `LITE-09-018` | `apps/server/src/services/AgentHistoryService.test.ts` | `passed` | 7 | CR6-A3 the q filter never searches message bodies (secrets excluded from search) | candidate-supported |
...
| `LITE-10-003` | `apps/server/src/services/RuntimeInspector.test.ts` | `passed` | 12 | INSP-12 Run, Stage, Provider, Process and duration stay distinct | candidate-supported |
| `LITE-10-004` | `apps/server/src/services/TaskRunService.test.ts` | `passed` | 92 | P3C1-RY-S01 Retry acceptance creates a queued Child and completed v3 Operation | candidate-supported |
...
| `LITE-10-008` | `apps/server/src/routes/runLifecycle.test.ts` | `passed` | 55 | P3C1-R27 no-key race: exactly one live 202, one stable 409 RUN_START_ALREADY_ACTIVE, and n | candidate-supported |
...
| `LITE-10-010` | `packages/process-runtime/src/durable-coordinator.test.ts` | `passed` | 34 | src/durable-coordinator.test.ts > DurableProcessCoordinator > establish creates exactly on | candidate-supported |
| `LITE-10-011` | `apps/server/src/services/WorkspaceAdmissionAuthority.test.ts` | `passed` | 46 | L1D-U03 frozen classifier, not requested class, is effective authority | candidate-supported |
| `LITE-10-012` | `packages/process-runtime/src/durable-coordinator.test.ts` | `passed` | 34 | — | insufficient-evidence |
| `LITE-10-013` | `packages/process-runtime/src/durable-coordinator.test.ts` | `passed` | 34 | src/durable-coordinator.test.ts > DurableProcessCoordinator > output writer persists only  | candidate-supported |
| `LITE-10-014` | `apps/server/src/store/SqliteStore.test.ts` | `passed` | 36 | tombstoned workspace rejected by assertWorkspaceExists for new Conversation | candidate-supported |
| `LITE-10-015` | `apps/server/src/migrations/__tests__/m2-7-workspace-compatibility.test.ts` | `passed` | 2 | — | insufficient-evidence |
| `LITE-10-016` | `apps/server/src/store/MemoryEntryRepository.test.ts` | `passed` | 16 | MF1R-15 record exposes no secret value field | candidate-supported |
...
| `LITE-11-002` | `apps/server/src/routes/conversationRuntime.test.ts` | `passed` | 9 | create-task and start-run bridge a Message into durable work | candidate-supported |
...
| `LITE-11-010` | `apps/server/src/routes/runLifecycle.test.ts` | `passed` | 55 | P3C1-RY06 live and replay return the immutable HTTP 201 acceptance snapshot | candidate-supported |
| `LITE-11-011` | `apps/server/src/routes/conversationRuntime.test.ts` | `passed` | 9 | history endpoint returns the agent unified references | candidate-supported |
...
| `LITE-12-011` | `apps/web/src/components/chat/RuntimeInspectorView.test.tsx` | `passed` | 7 | INS-07 Memory Context explains selection and exclusion from the frozen Snapshot | candidate-supported |
...
| `LITE-12-013` | `apps/server/src/services/AgentHistoryService.test.ts` | `passed` | 7 | CR6-A1 History unifies one Agent across Conversations, Messages, Turns, Tasks, Runs, Memor | candidate-supported |
...
| `LITE-13-002` | `apps/server/src/services/RuntimeInspector.test.ts` | `passed` | 12 | INSP-12 Run, Stage, Provider, Process and duration stay distinct | candidate-supported |
| `LITE-13-003` | `apps/server/src/services/RuntimeInspector.test.ts` | `passed` | 12 | INSP-12 Run, Stage, Provider, Process and duration stay distinct | candidate-supported |
| `LITE-13-004` | `apps/server/src/services/RuntimeInspector.test.ts` | `passed` | 12 | — | insufficient-evidence |
...
| `LITE-13-008` | `apps/server/src/services/TaskRunRecoveryService.test.ts` | `passed` | 67 | P6B recovery fixture uses the production repository seams | candidate-supported |
...
| `LITE-13-015` | `apps/server/src/services/RuntimeInspector.test.ts` | `passed` | 12 | — | insufficient-evidence |
