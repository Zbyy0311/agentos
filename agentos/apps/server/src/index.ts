...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    try {
      // Test-only crash-window seam. Absent by default, so the production
      // startup contract is unchanged; a spawned-server test sets it to prove
      // that a process death between the bootstrap COMMIT and L1D advancement
      // leaves a durable QUEUED Admission that the next startup reconverges.
      const failAfterBootstrapCommit =
        process.env.AGENTOS_TEST_FAIL_AFTER_L1E_BOOTSTRAP_BEFORE_ADVANCE === 'true';
      await new WorkspaceAdmissionStartupReconciler({
        store,
        testHooks: failAfterBootstrapCommit
          ? { afterBootstrapCommitBeforeAdvance: () => { throw new Error('TEST_CRASH_WINDOW'); } }
          : {},
      }).reconcileOnStartup();
    } catch (error) {
