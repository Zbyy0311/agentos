...
export function createPreferenceRoutes(store: SqliteStore, workspaceManager: WorkspaceManager, preferenceService = new PreferenceService(store)): Router {
  ...
  router.get('/preferences', (req: Request, res: Response) => {
    ...
    const context = typeof req.query.context === 'string' && contextKinds.has(req.query.context as PreferenceContextKind)
      ? req.query.context as PreferenceContextKind : undefined;
    const status = typeof req.query.status === 'string' && projectionStatuses.has(req.query.status as PreferenceProjectionStatus)
      ? req.query.status as PreferenceProjectionStatus : undefined;
    const projections = store.listPreferenceProjections(profile.id, workspace.id)
      .filter(item => (!context || item.contextKind === context) && (!status || item.status === status));
  ...
  router.post('/preferences/learning', (req: Request, res: Response) => {
    const workspace = workspaceManager.get(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ error: 'Workspace not found' });
    if (typeof req.body?.enabled !== 'boolean') return res.status(400).json({ error: 'enabled must be boolean' });
    const profile = req.body.enabled ? (preferenceService.resumeLearning('default'), store.getDefaultUserProfile()) : (preferenceService.pauseLearning('default'), store.getDefaultUserProfile());
    res.json({ profile });
  });

  router.post('/preferences/clear', (req: Request, res: Response) => {
    const workspace = workspaceManager.get(req.params.workspaceId);
    if (!workspace) return res.status(404).json({ error: 'Workspace not found' });
