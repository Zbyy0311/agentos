...
const FULL_MIGRATION_IDS = [...MIGRATION_IDS, '016', '017'];
...
function registryBefore012(): MigrationRegistry {
  return new MigrationRegistry(DEFAULT_REGISTRY_MIGRATIONS.filter(
    migration => migration.id !== '012' && migration.id !== '013' && migration.id !== '014' && migration.id !== '015' && migration.id !== '016' && migration.id !== '017',
  ));
}
...
function registryThrough015(): MigrationRegistry {
  return new MigrationRegistry(DEFAULT_REGISTRY_MIGRATIONS.filter(
    migration => migration.id !== '016' && migration.id !== '017',
  ));
}
