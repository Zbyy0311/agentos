...
function registryBefore012(): MigrationRegistry {
  return new MigrationRegistry(DEFAULT_REGISTRY_MIGRATIONS.filter(
    migration => migration.id !== '012' && migration.id !== '013' && migration.id !== '014' && migration.id !== '015' && migration.id !== '016' && migration.id !== '017' && migration.id !== '018' && migration.id !== '019' && migration.id !== '020' && migration.id !== '021' && migration.id !== '022' && migration.id !== '023',
  ));
}
