...
describe('M2.5 — Registry and integrity', () => {
  it('REG-01 registry IDs are exactly 001-009 in order with no duplicates', () => {
    const ids = DEFAULT_REGISTRY_MIGRATIONS.map((m) => m.id);
    assert.deepEqual(ids, ['001', '002', '003', '004', '005', '006', '007', '008', '009']);
  ...
  it('REG-03 migration records are exactly 001-009', () => {
    const db = migratedDb();
    try {
      const rows = db.prepare('SELECT migration_id FROM _schema_migrations ORDER BY migration_id').all() as Array<{ migration_id: string }>;
      assert.deepEqual(rows.map((r) => r.migration_id), ['001', '002', '003', '004', '005', '006', '007', '008', '009']);
    } finally {
      db.close();
    }
  });
