...
describe('M2.5 — Registry and integrity', () => {
  it('REG-01 registry IDs are exactly 001-016 in order with no duplicates', () => {
    const ids = DEFAULT_REGISTRY_MIGRATIONS.map((m) => m.id);
    assert.deepEqual(ids, ['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022', '023', '024', '025', '026', '027', '028', '029', '030', '031', '032', '033', '034']);
  ...
  // Title text is a live coverage marker for M2CoreAcceptance (REG-03); keep it
  // byte-stable even though the recorded registry now extends through 016.
  it('REG-03 migration records are exactly 001-014', () => {
    const db = migratedDb();
    try {
      const rows = db.prepare('SELECT migration_id FROM _schema_migrations ORDER BY migration_id').all() as Array<{ migration_id: string }>;
    assert.deepEqual(rows.map((r) => r.migration_id), ['001', '002', '003', '004', '005', '006', '007', '008', '009', '010', '011', '012', '013', '014', '015', '016', '017', '018', '019', '020', '021', '022', '023', '024', '025', '026', '027', '028', '029', '030', '031', '032', '033', '034']);
    } finally {
      db.close();
    }
  });
