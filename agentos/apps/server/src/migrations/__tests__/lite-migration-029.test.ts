...
test('S6/029: fresh and through028 upgrade apply identical additive 029', () => {
  ...
  try {
    ...
    // Compare at the 029 boundary; later additive migrations are outside this
    // historical upgrade proof.
    for (const migration of DEFAULT_REGISTRY_MIGRATIONS.filter(item => item.id <= '029')) migration.apply({ db: fresh });
