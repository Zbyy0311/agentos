...
test('[M27-P2-T001] JSON-only adoption writes Workspace, Agent, Provider and Completed Registry atomically', async () => {
  ...
  try {
    const source = writeSource(fx.root, [workspace('json-only', join(fx.root, 'json-only'))]);
    ...
    assert.equal((fx.db.prepare('SELECT COUNT(*) AS count FROM agent_profiles').get() as { count: number }).count, 1);
    assert.equal((fx.db.prepare('SELECT COUNT(*) AS count FROM provider_configurations').get() as { count: number }).count, 1);
