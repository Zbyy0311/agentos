...
export type LifecycleTransactionErrorCode =
  ...
  | 'LIFECYCLE_APPROVAL_HISTORY_INVALID';
...
interface TextStreamEventInputBase {
  readonly workspaceId: string;
  readonly runId: string;
  readonly stageId: string;
...
export type RecordTextStreamEventInput = TextStreamEventInputBase & (
  | { readonly type: 'stream.text_delta'; readonly payload: TextDeltaPayload }
  | { readonly type: 'stream.text_completed'; readonly payload: TextCompletedPayload }
);

export interface TextStreamEventResult {
  readonly run: Run;
  readonly stage: RunStage;
  readonly event: RuntimeEventEnvelope;
  readonly outbox: OutboxMessage;
...
interface StartupFailureInputBase extends CompositeLifecycleInputBase {
  readonly runId: string;
  readonly phase: string;
  readonly problem: ApiProblem;
}

export interface StartupFailureWithStageInput extends StartupFailureInputBase {
  readonly stageId: string;
  readonly expectedStageVersion: number;
}

export interface StartupFailureWithoutStageInput extends StartupFailureInputBase {
  readonly stageId?: never;
  readonly expectedStageVersion?: never;
}

export type FailRunStartupInput = StartupFailureWithStageInput | StartupFailureWithoutStageInput;

export interface StartStageInput extends StageCompositeInput {
  readonly runId: string;
  readonly agentSnapshot: AgentSnapshotV1;
  readonly providerSnapshot: ProviderConfigurationSnapshotV1;
}
...
export interface CancelRunForOperationWithinTransactionInput {
  readonly workspaceId: string;
  readonly runId: string;
  readonly correlationId: string;
}
...
type P6BRecoveryStatus = Extract<
  M3RunStatus,
  'queued' | 'running' | 'waiting_approval' | 'paused'
>;

type P6BRecoveryOutcome =
  | {
      readonly kind: 'recovered';
      readonly recoveryMode: 'queue-restore' | 'approval-restore';
    }
  | {
...
export interface RecordRecoveryOutcomeWithinTransactionInput extends CompositeLifecycleInputBase {
  readonly runId: string;
  readonly expectedStatus: P6BRecoveryStatus;
  readonly processFound: boolean;
  readonly providerSessionFound: boolean;
  readonly worktreeFound: boolean;
  readonly outcome: P6BRecoveryOutcome;
}
...
export class LifecycleTransactionService {
  ...
  recordTextStreamEvent(input: RecordTextStreamEventInput): TextStreamEventResult {
    return this.dependencies.runInTransaction(() => this.recordTextStreamEventWithinTransaction(input));
  }

  recordTextStreamEventWithinTransaction(input: RecordTextStreamEventInput): TextStreamEventResult {
    this.validateTextStreamEventInput(input);
    ...
    const event = this.dependencies.runtimeEventRepository.appendWithinTransaction(draft);
    const outbox = this.insertOutbox(event, timestamp);
    return { run: this.requireRun(input.workspaceId, input.runId), stage, event, outbox };
  }

  transitionRun(input: RunTransitionInput): RunLifecycleTransitionResult {
    return this.dependencies.runInTransaction(() => this.transitionRunWithinTransaction(input));
  }

  transitionRunWithinTransaction(input: RunTransitionInput): RunLifecycleTransitionResult {
  ...
  transitionStage(input: StageTransitionInput): StageLifecycleTransitionResult {
    return this.dependencies.runInTransaction(() => this.transitionStageWithinTransaction(input));
  }

  transitionStageWithinTransaction(input: StageTransitionInput): StageLifecycleTransitionResult {
  ...
  completeRunStartup(input: CompleteRunStartupInput): CompositeLifecycleTransactionResult {
    ...
    return this.dependencies.runInTransaction(() => this.completeRunStartupWithinTransaction(input));
  }

  completeRunStartupWithinTransaction(input: CompleteRunStartupInput): CompositeLifecycleTransactionResult {
    this.validateCompleteRunStartupInput(input);
    ...
    this.assertSingleStartupStage(input.workspaceId, input.runId, stage.id);
  ...
  failRunStartup(input: FailRunStartupInput): CompositeLifecycleTransactionResult {
    this.validateFailRunStartupInput(input);
    return this.dependencies.runInTransaction(() => this.failRunStartupWithinTransaction(input));
  }

  failRunStartupWithinTransaction(input: FailRunStartupInput): CompositeLifecycleTransactionResult {
    this.validateFailRunStartupInput(input);
    this.assertStartupFailureOrderingContract();
    const expectedRunVersion = this.expectedRunVersion(input);
    const run = this.requireRun(input.workspaceId, input.runId);
    this.assertExpectedRunState(run, 'starting');
    this.assertExpectedVersion('runs', run.id, run.version, expectedRunVersion);
    this.assertProblemBinding(input);
    const timestamp = this.transactionTimestamp();
    const failure = {
      errorCode: input.problem.code,
      message: input.problem.detail,
      phase: input.phase,
      retryable: input.problem.retryable,
    };
    ...
    if (input.stageId !== undefined) {
      ...
      return this.compositeResult(
        ...
        input.runId,
        [stageEvent, runEvent],
        [stageOutbox, runOutbox],
      );
    }

    const stages = this.dependencies.runStageRepository.listByRun(input.workspaceId, input.runId);
    if (stages.some(stage => stage.status === 'starting' || stage.status === 'running' || stage.startedAt !== undefined)) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_STATE_MISMATCH',
        `Run ${run.id} has a Stage that already entered startup`,
      );
    }
    const nextRun = this.dependencies.runRepository.transitionLifecycleWithinTransaction({
      workspaceId: input.workspaceId,
    ...
    const runOutbox = this.insertOutbox(runEvent, timestamp);
    return this.compositeResult(input.workspaceId, input.runId, [runEvent], [runOutbox]);
  }

  recordRecoveryOutcomeWithinTransaction(
    input: RecordRecoveryOutcomeWithinTransactionInput,
  ): CompositeLifecycleTransactionResult {
    this.validateRecoveryOutcomeInput(input);
    const expectedRunVersion = this.expectedRunVersion(input);
    const run = this.requireRun(input.workspaceId, input.runId);
    this.assertExpectedRunState(run, input.expectedStatus);
    this.assertExpectedVersion('runs', run.id, run.version, expectedRunVersion);
    if (run.recoveryRequired === true) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_STATE_MISMATCH',
        `Run ${run.id} already requires reviewed recovery`,
      );
    }

    const timestamp = this.transactionTimestamp();
    const attempted = this.appendEvent(
      run,
      undefined,
      ...
      input.metadata,
      {
        ...
        processFound: input.processFound,
        providerSessionFound: input.providerSessionFound,
        worktreeFound: input.worktreeFound,
      },
    );
    const attemptedOutbox = this.insertOutbox(attempted, timestamp);

    let currentRun = run;
    let outcomeType: 'run.recovered' | 'run.recovery_failed';
    let outcomePayload: Record<string, unknown>;
    if (input.outcome.kind === 'recovered') {
      outcomeType = 'run.recovered';
      outcomePayload = { recoveryMode: input.outcome.recoveryMode };
    } else {
      currentRun = this.dependencies.runRepository.markRecoveryRequiredWithinTransaction({
        workspaceId: input.workspaceId,
        runId: input.runId,
      ...
      outcomeType = 'run.recovery_failed';
      outcomePayload = {
        errorCode: input.outcome.errorCode,
        message: input.outcome.message,
        retryableAsNewRun: input.outcome.retryableAsNewRun,
      };
    }

    const outcome = this.appendEvent(
      currentRun,
      undefined,
      outcomeType,
      timestamp,
    ...
    const outcomeOutbox = this.insertOutbox(outcome, timestamp);
    return this.compositeResult(
      input.workspaceId,
      input.runId,
      [attempted, outcome],
      [attemptedOutbox, outcomeOutbox],
    );
  }

  startStage(input: StartStageInput): StageLifecycleTransitionResult {
    this.validateStartStageInput(input);
    return this.dependencies.runInTransaction(() => {
      ...
      this.assertExpectedVersion('run_stages', stage.id, stage.version, input.expectedStageVersion);
      const timestamp = this.transactionTimestamp();
      const nextStage = this.dependencies.runStageRepository.transitionLifecycleWithinTransaction({
        workspaceId: input.workspaceId,
        runId: input.runId,
      ...
      const outbox = this.insertOutbox(event, timestamp);
      return {
        run: this.requireRun(input.workspaceId, input.runId),
        stage: nextStage,
        event,
        outbox,
      };
  ...
  /**
   * Cancels the Run bound to a persisted Operation inside the caller-owned
   * transaction. The Operation API supplies only the persisted binding; all
   * lifecycle metadata and expected aggregate versions are derived here.
   */
  cancelRunForOperationWithinTransaction(
    input: CancelRunForOperationWithinTransactionInput,
  ): CompositeLifecycleTransactionResult {
    this.validateOperationCancellationInput(input);
    const run = this.requireRun(input.workspaceId, input.runId);
    ...
    const trusted = {
      ...
      correlationId: input.correlationId,
      requestedBy: 'operation_api' as const,
      terminatedProcessIds: [] as string[],
  ...
  completeRun(input: CompleteRunInput): CompositeLifecycleTransactionResult {
    return this.dependencies.runInTransaction(() => this.completeRunWithinTransaction(input));
  }

  completeRunWithinTransaction(input: CompleteRunInput): CompositeLifecycleTransactionResult {
  ...
  private validateTextStreamEventInput(input: RecordTextStreamEventInput): void {
    if (
      !isNonBlankString(input.workspaceId)
      || !isNonBlankString(input.runId)
      || !isNonBlankString(input.stageId)
      || !isNonBlankString(input.correlationId)
    ) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_VALIDATION_FAILED',
        'workspaceId, runId, stageId, and correlationId are required',
      );
    }
    this.validateOptionalString(input.agentId, 'agentId');
    this.validateOptionalString(input.providerConfigId, 'providerConfigId');
    if (input.type !== 'stream.text_delta' && input.type !== 'stream.text_completed') {
      throw new LifecycleTransactionError('LIFECYCLE_VALIDATION_FAILED', 'text stream Event type is invalid');
    }
  }

  private validateOperationCancellationInput(input: CancelRunForOperationWithinTransactionInput): void {
    if (!isNonBlankString(input.workspaceId) || !isNonBlankString(input.runId)
      || !isNonBlankString(input.correlationId)) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_VALIDATION_FAILED',
  ...
  private validateFailRunStartupInput(input: FailRunStartupInput): void {
    this.validateCompositeCommonInput(input);
    this.validateRunId(input.runId);
    if (!isNonBlankString(input.phase)) {
      throw new LifecycleTransactionError('LIFECYCLE_VALIDATION_FAILED', 'phase is required');
    }
    if (!isValidApiProblem(input.problem)) {
      throw new LifecycleTransactionError('LIFECYCLE_VALIDATION_FAILED', 'startup failure problem is malformed');
    }
    if (input.stageId === undefined) {
      if (input.expectedStageVersion !== undefined) {
        throw new LifecycleTransactionError(
          'LIFECYCLE_VALIDATION_FAILED',
  ...
  private validateRecoveryOutcomeInput(input: RecordRecoveryOutcomeWithinTransactionInput): void {
    this.validateCompositeCommonInput(input);
    this.validateRunId(input.runId);
    if (!['queued', 'running', 'waiting_approval', 'paused'].includes(input.expectedStatus)) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_VALIDATION_FAILED',
        'recovery expectedStatus must be non-terminal',
      );
    }
    if (typeof input.processFound !== 'boolean'
      || typeof input.providerSessionFound !== 'boolean'
      || typeof input.worktreeFound !== 'boolean') {
      throw new LifecycleTransactionError(
        'LIFECYCLE_VALIDATION_FAILED',
        'recovery evidence flags must be boolean',
      );
    }
    if (!isRecord(input.outcome)) {
      throw new LifecycleTransactionError('LIFECYCLE_VALIDATION_FAILED', 'recovery outcome is required');
    }
    if (input.outcome.kind === 'recovered') {
      const expectedMode = input.expectedStatus === 'queued'
        ? 'queue-restore'
        : input.expectedStatus === 'waiting_approval'
          ? 'approval-restore'
          : undefined;
      if (input.outcome.recoveryMode !== expectedMode) {
        throw new LifecycleTransactionError(
          'LIFECYCLE_VALIDATION_FAILED',
          'recoveryMode does not match the persisted Run status',
        );
      }
      return;
    }
    if (input.outcome.kind !== 'failed'
      || !['running', 'waiting_approval', 'paused'].includes(input.expectedStatus)
      || !isNonBlankString(input.outcome.errorCode)
      || !isNonBlankString(input.outcome.message)
      || typeof input.outcome.retryableAsNewRun !== 'boolean') {
      throw new LifecycleTransactionError(
        'LIFECYCLE_VALIDATION_FAILED',
        'recovery failure outcome is invalid',
      );
    }
  }

  private validateStartStageInput(input: StartStageInput): void {
    this.validateCompositeCommonInput(input);
    this.validateRunAndStageIds(input.runId, input.stageId);
    this.expectedStageVersion(input);
    if (!isRecord(input.agentSnapshot) || !isRecord(input.providerSnapshot)) {
      throw new LifecycleTransactionError('LIFECYCLE_VALIDATION_FAILED', 'stage startup snapshots are required');
    }
  }

  private assertStartupFailureOrderingContract(): void {
    const contract = M3_MULTI_EVENT_ORDERING_CONTRACTS.find(item => item.name === 'startup-failure');
    if (
      !contract
      || contract.events.join(',') !== 'stage.failed,run.failed'
      || contract.stageMultiplicity !== 'single'
      || contract.stageOrdering !== 'none'
      || !contract.contiguousRunSequence
      || !contract.independentOutboxPerEvent
      || !contract.atomicCurrentStateEventOutbox
    ) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_VALIDATION_FAILED',
        'Shared startup-failure ordering contract is invalid',
      );
    }
  }

  private assertProblemBinding(input: FailRunStartupInput): void {
    const context = input.problem.context;
    if (
      !context
      || context.runId !== input.runId
      || context.operationId !== input.correlationId
      || (context.workspaceId !== undefined && context.workspaceId !== input.workspaceId)
      || (input.stageId === undefined ? context.stageId !== undefined : context.stageId !== input.stageId)
    ) {
      throw new LifecycleTransactionError('LIFECYCLE_VALIDATION_FAILED', 'startup failure Stage binding is invalid');
    }
  }
  ...
  private discoverOperationApproval(
    run: Run,
    workspaceId: string,
  ): { approvalRequestId: string; stageId?: string } {
    let records: ReturnType<RuntimeEventRepository['listByRunAfterSequence']>;
    try {
      records = this.dependencies.runtimeEventRepository.listByRunAfterSequence(run.id, 0);
    } catch {
      throw new LifecycleTransactionError(
        'LIFECYCLE_APPROVAL_HISTORY_INVALID',
        `Run ${run.id} approval history could not be safely consumed`,
      );
    }
    const required = new Map<string, RuntimeEventEnvelope>();
    const resolved = new Map<string, RuntimeEventEnvelope>();
    ...
    for (const record of records) {
      if (record.kind !== 'known') {
        throw new LifecycleTransactionError(
          'LIFECYCLE_APPROVAL_HISTORY_INVALID',
          `Run ${run.id} contains an unknown relevant Runtime Event`,
        );
      }
      const event = record.event;
      if (event.workspaceId !== workspaceId || event.runId !== run.id || event.sequence !== expectedSequence) {
        throw new LifecycleTransactionError(
          'LIFECYCLE_APPROVAL_HISTORY_INVALID',
          `Run ${run.id} approval history binding or sequence is invalid`,
        );
      }
      expectedSequence += 1;
      if (event.type !== 'approval.required' && event.type !== 'approval.resolved') continue;
      if (!isNonBlankString(event.approvalRequestId)) {
        throw new LifecycleTransactionError(
          'LIFECYCLE_APPROVAL_HISTORY_INVALID',
          `Run ${run.id} approvalRequestId is invalid`,
        );
      }
      if (event.stageId !== undefined) {
        if (!isNonBlankString(event.stageId)
          || !this.dependencies.runStageRepository.findById(workspaceId, run.id, event.stageId)) {
          throw new LifecycleTransactionError(
            'LIFECYCLE_APPROVAL_HISTORY_INVALID',
            `Run ${run.id} approval Stage binding is invalid`,
          );
        }
      }

      if (event.type === 'approval.required') {
        if (required.has(event.approvalRequestId)) {
          throw new LifecycleTransactionError(
            'LIFECYCLE_APPROVAL_HISTORY_INVALID',
            `Run ${run.id} contains duplicate approval.required history`,
          );
        }
        required.set(event.approvalRequestId, event);
        continue;
      }

      const requiredEvent = required.get(event.approvalRequestId);
      if (!requiredEvent || resolved.has(event.approvalRequestId)
        || (requiredEvent.stageId ?? undefined) !== (event.stageId ?? undefined)) {
        throw new LifecycleTransactionError(
          'LIFECYCLE_APPROVAL_HISTORY_INVALID',
          `Run ${run.id} approval.resolved history is not bound to one required approval`,
        );
      }
      resolved.set(event.approvalRequestId, event);
    }

    const unresolved = [...required.entries()]
      .filter(([approvalRequestId]) => !resolved.has(approvalRequestId));
    if (unresolved.length !== 1) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_APPROVAL_HISTORY_INVALID',
        `Run ${run.id} must contain exactly one unresolved approval.required history`,
      );
    }
    const [approvalRequestId, event] = unresolved[0]!;
    return {
      approvalRequestId,
  ...
  private assertSingleStartupStage(workspaceId: string, runId: string, stageId: string): void {
    const stages = this.dependencies.runStageRepository.listByRun(workspaceId, runId);
    const startingStages = stages.filter(stage => stage.status === 'starting');
    if (
      startingStages.length !== 1
      || startingStages[0]?.id !== stageId
      || stages.some(stage => stage.status === 'running')
    ) {
      throw new LifecycleTransactionError(
        'LIFECYCLE_STATE_MISMATCH',
        `Run ${runId} does not have exactly one startup Stage ${stageId}`,
      );
    }
  }
