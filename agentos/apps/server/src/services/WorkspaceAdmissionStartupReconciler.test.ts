...
/** Insert a Run that carries a canonical failure code (for RUN_PROCESS_MISSING). */
function insertFailedRun(
  ...
  insertRun(store, workspaceId, runId, 'failed', createdAt);
  store.getDatabase().prepare(
    'UPDATE runs SET failure_code = ?, failure_message = ? WHERE workspace_id = ? AND id = ?',
  ).run(failureCode, 'seeded failure', workspaceId, runId);
}
...
// --- HIGH-1: crash-safe durable advancement workset ---

// L1E-U17: bootstrap COMMIT -> advancement crash window is reconverged by the
// NEXT startup, which derives its workset from durable state rather than from
// an in-memory changed-this-run flag.
test('L1E-U17 pending Admission left by a crash is advanced by the next startup', async () => {
  const root = makeTempRoot('u17');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-q1', 'queued', '2026-07-25T00:00:01.000Z');

    // Startup #1: the crash seam fires after the bootstrap transaction has
    // COMMITTED and before L1D advancement runs.
    const crashed = new WorkspaceAdmissionStartupReconciler({
      store,
      testHooks: { afterBootstrapCommitBeforeAdvance: () => { throw new Error('CRASH_WINDOW'); } },
    });
    await assert.rejects(() => crashed.reconcileOnStartup());

    // Durable proof: the QUEUED Admission survived the crash.
    const afterCrash = admissionsFor(store, 'ws-a');
    assert.equal(afterCrash.length, 1);
    assert.equal(afterCrash[0].state, 'QUEUED');
    assert.equal(afterCrash[0].queueReason, 'WAITING_FOR_WORKSPACE_ADMISSION');
    assert.equal(afterCrash[0].requestOrder, 1);
    assert.equal(afterCrash[0].version, 1);
    const crashedId = afterCrash[0].id;

    // Startup #2: a brand-new reconciler (simulating a fresh process) reads the
    // durable pending workset and converges the Admission through L1D.
    await reconcile(store);
    const afterRestart = admissionsFor(store, 'ws-a');
    assert.equal(afterRestart.length, 1, 'no duplicate Admission is created');
    assert.equal(afterRestart[0].id, crashedId, 'the same Admission converges in place');
    assert.equal(afterRestart[0].requestOrder, 1, 'request_order is unchanged');
    assert.equal(afterRestart[0].state, 'GRANTED');
    assert.ok(afterRestart[0].grantedAt !== null);
    assert.equal(afterRestart[0].version, 2, 'only the legitimate QUEUED->GRANTED CAS bumps version');
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
  }
...
// L1E-R09: the durable workset is the post-commit pending set, independent of
// whether this invocation inserted the row. A pre-existing durable QUEUED
// Admission with no in-memory "changed" record still converges.
test('L1E-R09 advancement workset comes from durable pending rows', async () => {
  const root = makeTempRoot('r09');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-q1', 'queued', '2026-07-25T00:00:01.000Z');
    // Seed the Admission directly: the reconciler inserts nothing this run.
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-seeded1', runId: 'run-q1', requestOrder: 1, state: 'QUEUED',
    });
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows.length, 1);
    assert.equal(rows[0].id, 'grant-seeded1');
    assert.equal(rows[0].state, 'GRANTED', 'a pre-existing QUEUED row is converged by the durable workset');
    assert.equal(rows[0].requestOrder, 1);
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
...
// --- HIGH-2: terminal subject + GRANTED Admission releases through L1D ---

// L1E-U18: terminal canonical subject + GRANTED holder -> RELEASED via L1D.
test('L1E-U18 terminal subject with GRANTED Admission is released on startup', async () => {
  const root = makeTempRoot('u18');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-done', 'completed', '2026-07-25T00:00:01.000Z');
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-phantom', runId: 'run-done', requestOrder: 1, state: 'GRANTED',
      grantedAt: '2026-07-25T00:00:02.000Z',
    });
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows.length, 1);
    assert.equal(rows[0].state, 'RELEASED', 'a terminal subject never keeps a GRANTED holder');
    assert.equal(rows[0].releaseReason, 'RUN_TERMINAL');
    assert.ok(rows[0].releasedAt !== null);
...
// L1E-U19: RUN_PROCESS_MISSING terminal keeps the L1D release-reason vocabulary.
test('L1E-U19 RUN_PROCESS_MISSING terminal releases with the L1D missing reason', async () => {
  const root = makeTempRoot('u19');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertFailedRun(store, 'ws-a', 'run-missing', 'RUN_PROCESS_MISSING', '2026-07-25T00:00:01.000Z');
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-missing', runId: 'run-missing', requestOrder: 1, state: 'GRANTED',
      grantedAt: '2026-07-25T00:00:02.000Z',
    });
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows[0].state, 'RELEASED');
    assert.equal(rows[0].releaseReason, 'RUN_PROCESS_MISSING_TERMINAL',
      'release reason comes from L1D, never fabricated here');
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
  }
...
// L1E-U20: terminal GRANTED holder + queued follower -> release, then the
// follower wins through L1D advancement.
test('L1E-U20 terminal holder release unblocks the queued follower', async () => {
  const root = makeTempRoot('u20');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-done', 'completed', '2026-07-25T00:00:01.000Z');
    insertRun(store, 'ws-a', 'run-next', 'queued', '2026-07-25T00:00:02.000Z');
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-holder', runId: 'run-done', requestOrder: 1, state: 'GRANTED',
      grantedAt: '2026-07-25T00:00:02.000Z',
    });
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-follower', runId: 'run-next', requestOrder: 2, state: 'QUEUED',
    });
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows.length, 2);
    assert.equal(rows[0].id, 'grant-holder');
    assert.equal(rows[0].state, 'RELEASED');
    assert.equal(rows[1].id, 'grant-follower');
    assert.equal(rows[1].state, 'GRANTED', 'the queued follower advances after the phantom holder is released');
    assert.equal(rows[1].requestOrder, 2, 'request_order is preserved');
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
  }
...
// L1E-U21: a second startup performs zero repeated release and zero
// unjustified version churn.
test('L1E-U21 terminal release is idempotent across restarts', async () => {
  const root = makeTempRoot('u21');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-done', 'completed', '2026-07-25T00:00:01.000Z');
    ...
    await reconcile(store);
    const first = admissionsFor(store, 'ws-a').map(r => ({
      id: r.id, state: r.state, order: r.requestOrder, version: r.version,
      releaseReason: r.releaseReason, releasedAt: r.releasedAt,
    }));
    await reconcile(store);
    const second = admissionsFor(store, 'ws-a').map(r => ({
      id: r.id, state: r.state, order: r.requestOrder, version: r.version,
      releaseReason: r.releaseReason, releasedAt: r.releasedAt,
    }));
    assert.deepEqual(second, first, 'restart must not re-release or churn versions');
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
  }
...
// L1E-U22: terminal + already-terminal Admission stays unchanged.
test('L1E-U22 terminal subject with terminal Admission is untouched', async () => {
  const root = makeTempRoot('u22');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-done', 'completed', '2026-07-25T00:00:01.000Z');
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-done', runId: 'run-done', requestOrder: 1, state: 'RELEASED',
      grantedAt: '2026-07-25T00:00:02.000Z', releaseReason: 'RUN_TERMINAL',
      releasedAt: '2026-07-25T00:00:03.000Z',
    });
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows[0].state, 'RELEASED');
    assert.equal(rows[0].version, 1, 'a terminal Admission is never rewritten');
    assert.equal(rows[0].releasedAt, '2026-07-25T00:00:03.000Z');
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
  }
...
// L1E-U23: a GRANTED Admission whose subject row is gone fails closed instead
// of silently leaking a phantom holder.
test('L1E-U23 GRANTED Admission with a dangling subject fails closed', async () => {
  const root = makeTempRoot('u23');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-done', 'completed', '2026-07-25T00:00:01.000Z');
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-dangling', runId: 'run-done', requestOrder: 1, state: 'GRANTED',
      grantedAt: '2026-07-25T00:00:02.000Z',
    });
    // FK RESTRICT prevents deleting the Run, so simulate the dangling binding
    // by dropping the row's subject reference through a constraint bypass.
    const db = store.getDatabase();
    db.exec('PRAGMA foreign_keys = OFF');
    db.prepare('DELETE FROM runs WHERE workspace_id = ? AND id = ?').run('ws-a', 'run-done');
    db.exec('PRAGMA foreign_keys = ON');
    await assert.rejects(() => reconcile(store), WorkspaceAdmissionStartupReconciliationError);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows[0].state, 'GRANTED', 'fail closed without mutating the row');
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
  }
...
// --- MEDIUM-2: cross-kind deterministic bootstrap order ---

// L1E-U26: a release can promote a follower whose subject is ALSO terminal.
// The scan must iterate to a fixed point so no terminal subject keeps a
// GRANTED Admission after one startup.
test('L1E-U26 terminal release converges when the promoted follower is terminal too', async () => {
  const root = makeTempRoot('u26');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    insertRun(store, 'ws-a', 'run-done', 'completed', '2026-07-25T00:00:01.000Z');
    insertRun(store, 'ws-a', 'run-also-done', 'cancelled', '2026-07-25T00:00:02.000Z');
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-holder', runId: 'run-done', requestOrder: 1, state: 'GRANTED',
      grantedAt: '2026-07-25T00:00:02.000Z',
    });
    insertAdmissionRow(store, 'ws-a', {
      admissionId: 'grant-follower', runId: 'run-also-done', requestOrder: 2, state: 'QUEUED',
    });
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows.length, 2);
    for (const row of rows) {
      assert.equal(row.state, 'RELEASED', row.id + ' must not keep a GRANTED Admission');
      assert.equal(row.releaseReason, 'RUN_TERMINAL');
      assert.ok(row.releasedAt !== null);
    }
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
...
// L1E-U24: with equal executing group and equal created_at, a legacy id that
// sorts BEFORE the canonical id must still receive the HIGHER request_order.
// This proves the kind tie-break rather than an accidental lexicographic win.
test('L1E-U24 cross-kind tie-break prefers CANONICAL_RUN before LEGACY_AGENT_RUN', async () => {
  const root = makeTempRoot('u24');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    const sameCreatedAt = '2026-07-25T00:00:01.000Z';
    // 'aaa-legacy' < 'zzz-canonical' lexicographically, so an ID-first
    // comparator would give the legacy subject the lower request_order.
    insertLegacyRun(store, 'ws-a', 'aaa-legacy', 'queued', sameCreatedAt);
    insertRun(store, 'ws-a', 'zzz-canonical', 'queued', sameCreatedAt);
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows.length, 2);
    assert.equal(rows[0].subjectKind, 'CANONICAL_RUN');
    assert.equal(rows[0].canonicalRunId, 'zzz-canonical');
    assert.equal(rows[0].requestOrder, 1, 'canonical kind sorts before legacy');
    assert.equal(rows[1].subjectKind, 'LEGACY_AGENT_RUN');
    assert.equal(rows[1].legacyRunId, 'aaa-legacy');
    assert.equal(rows[1].requestOrder, 2, 'legacy id ordering must not override the kind rule');
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
  }
...
// L1E-U25: subjectId still breaks ties WITHIN the same kind.
test('L1E-U25 subjectId orders bootstrap within the same kind', async () => {
  const root = makeTempRoot('u25');
  const store = new SqliteStore(root);
  try {
    seedWorkspace(store, root, 'ws-a');
    const sameCreatedAt = '2026-07-25T00:00:01.000Z';
    insertRun(store, 'ws-a', 'run-b', 'queued', sameCreatedAt);
    insertRun(store, 'ws-a', 'run-a', 'queued', sameCreatedAt);
    await reconcile(store);
    const rows = admissionsFor(store, 'ws-a');
    assert.equal(rows[0].canonicalRunId, 'run-a');
    assert.equal(rows[0].requestOrder, 1);
    assert.equal(rows[1].canonicalRunId, 'run-b');
    assert.equal(rows[1].requestOrder, 2);
  } finally {
    store.close();
    rmSync(root, { recursive: true, force: true });
