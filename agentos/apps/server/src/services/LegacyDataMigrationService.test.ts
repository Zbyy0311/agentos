...
async function createFixture(): Promise<Fixture> {
  ...
  const { parseLegacyJsonSource } = await import('./LegacySourceParser.js') as {
    parseLegacyJsonSource(bytes: Uint8Array, sourceKey: 'workspaces.json' | 'tasks.json'): unknown;
  };
  ...
  const service = new LegacyDataMigrationService({
    ...
    clock: () => NOW,
    parser: (bytes: Uint8Array, sourceKey: 'workspaces.json' | 'tasks.json') => {
      state.parserCalls += 1;
      calls.push('parser');
      return parseLegacyJsonSource(bytes, sourceKey);
    },
  });
...
test('[M27-P1-T009] Source-only hash branch creates a new Completed Attempt and reuses the accepted Revision', async () => {
  ...
  try {
    const first = await fixture.run(Buffer.from('{"tasks":[{"id":"task-1","value":1}]}', 'utf8'));
    const second = await fixture.run(Buffer.from(" { \"tasks\": [ { \"value\": 1, \"id\": \"task-1\" }\n] }", 'utf8'));
...
test('[M27-P1-T010] Payload Hash branch creates the next accepted Revision', async () => {
  const fixture = await createFixture();
  try {
    await fixture.run(Buffer.from('{"tasks":[{"id":"task-1","value":1}]}', 'utf8'));
    const changed = await fixture.run(Buffer.from('{"tasks":[{"id":"task-1","value":2}]}', 'utf8'));
...
test('[M27-P1-T040] exact-source no-op returns old evidence and creates no Attempt', async () => {
  const fixture = await createFixture();
  try {
    const source = Buffer.from('{"tasks":[{"id":"task-1","value":1}]}', 'utf8');
...
test('[M27-P1-T041] exact-source no-op calls neither SQLite nor JSON Backup Provider', async () => {
  const fixture = await createFixture();
  try {
    const source = Buffer.from('{"tasks":[{"id":"task-1"}]}', 'utf8');
...
test('[M27-P1-T042] no-op miss verifies Backup before Attempt Reservation', async () => {
  ...
  try {
    const order: string[] = [];
    const result = await fixture.run(Buffer.from('{"tasks":[{"id":"task-1"}]}', 'utf8'), async context => {
...
test('[M27-P1-T044] Source-only hash change reuses the latest accepted Revision', async () => {
  ...
  try {
    await fixture.run(Buffer.from('{"tasks":[{"id":"task-1","value":1}]}', 'utf8'));
    await fixture.run(Buffer.from('{"tasks":[{"id":"task-1","value":2}]}', 'utf8'));
    const sourceOnly = await fixture.run(Buffer.from(' { "tasks": [ { "value": 2, "id": "task-1" } ] } ', 'utf8'));
...
test('[M27-P1-T045] an older Payload returning after a newer accepted Payload creates latest Revision plus one', async () => {
  ...
  try {
    ...
    const historical = await fixture.run(Buffer.from(' { "tasks": [ { "value": 1, "id": "task-1" } ] } ', 'utf8'));
