    ...
    <main className="mx-auto w-full max-w-7xl px-5 py-10 sm:px-8 lg:py-16">
      ...
      <section className="signal-panel ui-panel-raised rounded-2xl border p-4 sm:p-6">
        ...
        <div className="mb-5 flex flex-col gap-2 rounded-xl border ui-border bg-[color:var(--app-bg)]/45 p-2 sm:flex-row">
          <input suppressHydrationWarning aria-label="要导入的本地目录路径" type="text" value={importPath} onChange={event => setImportPath(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void handleImport(); }} placeholder="输入本地目录路径以导入…" className="ui-input min-w-0 flex-1 rounded-xl px-3.5 py-2.5 text-sm outline-none" />
