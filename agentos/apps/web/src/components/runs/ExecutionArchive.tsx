...
export function ExecutionArchive({ details }: { details: AgentRunDetails }) {
  ...
  return <section aria-label="Execution archive" className="space-y-3">
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="w-44"><CompactSelect label="类型" value={kind} options={[{ value: 'all', label: '全部类型' }, ...kinds.map(value => ({ value, label: value }))]} onChange={value => setKind(value as ArchiveItemKind | 'all')} /></div>
