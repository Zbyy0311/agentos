...
export function ArtifactPreviewDialog({ artifact, url, onClose }: { artifact: RuntimeArtifact; url: string; onClose(): void }) {
  ...
  const glassRef = useLiquidGlass<HTMLElement>('modal');
  ...
  useEffect(() => {
    ...
    void fetch(url, { signal: controller.signal }).then(async response => {
      if (!response.ok) {
        setLoadError(response.status === 404 ? '产物不存在或已被清理。' : response.status === 403 ? '当前工作区无权读取该产物。' : `产物加载失败（${response.status}）。`);
        return;
      }
    ...
    }).catch(() => setLoadError('无法连接到产物服务。'));
    return () => controller.abort();
  }, [isImage, url]);
  return <div className={`fixed inset-0 ${uiLayerClass('mediaPreview')} grid place-items-center bg-black/50 p-6`} role="dialog" aria-modal="true" aria-label="产物预览" onClick={onClose}>
    <section ref={glassRef} className="glass-dialog ui-panel max-h-[90vh] w-full max-w-4xl overflow-auto rounded-2xl border p-4 shadow-[var(--app-shadow)]" onClick={event => event.stopPropagation()}>
