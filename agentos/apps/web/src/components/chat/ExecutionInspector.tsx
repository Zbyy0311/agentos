...
export function ExecutionInspector({ agent, groupTitle, events, runtimeEvents = [], steps = [], executions, activeStatus, activeStartedAt, onEdit, onOpenRunDetails }: ExecutionInspectorProps) {
  ...
  return <aside data-signal-inspector className="inspector-sidebar signal-inspector ui-panel w-64 shrink-0 overflow-y-auto border-l px-4 py-5">
    ...
    {(agent || groupTitle) && <>
      ...
      <section className="mt-5 grid grid-cols-3 gap-1.5 border-t ui-border pt-5" aria-label="执行统计"><Stat label="消耗" value={`Tokens ${formatTokens(summary.usage?.totalTokens)}`} /><Stat label="Duration" value={formatDuration(summary.durationMs ?? (elapsedStartedAt ? elapsed * 1000 : undefined))} /><Stat label="Files" value={`+${summary.files.added} -${summary.files.removed}${summary.files.changed ? ` ~${summary.files.changed}` : ''}`} /></section>
...
function Stat({ label, value }: { label: string; value: string }) { return <div className="rounded-lg border ui-border px-2 py-2"><div className="text-[10px] ui-dim">{label}</div><div className="mt-1 text-xs font-semibold ui-text">{value}</div></div>; }
function formatTokens(value: number | undefined): string { return value === undefined ? '—' : value >= 1000 ? `${(value / 1000).toFixed(1).replace(/\.0$/, '')}k` : String(value); }
