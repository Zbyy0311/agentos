...
export function ExecutionInspector({ agent, groupTitle, events, executions, activeStatus, activeStartedAt, onEdit, onOpenRunDetails }: ExecutionInspectorProps) {
  ...
  const elapsed = elapsedStartedAt
    ? getElapsedSeconds({ startedAt: elapsedStartedAt, completedAt: activeStartedAt ? undefined : latest?.completedAt })
    : 0;
  ...
  return <aside className="inspector-sidebar ui-panel w-64 shrink-0 overflow-y-auto border-l px-4 py-5">
    ...
    {(agent || groupTitle) && <><section className="border-t ui-border pt-5"><div className="mb-4 flex items-center justify-between"><h3 className="text-[11px] font-medium tracking-[0.14em] ui-dim">TIMELINE</h3>{status && <span className="text-xs ui-muted">{statusLabel[status]}</span>}</div>{latest?.runId && onOpenRunDetails && <button type="button" onClick={() => onOpenRunDetails(latest.runId)} className="mb-4 w-full rounded-lg border border-[var(--app-accent)]/40 px-3 py-2 text-left text-xs ui-accent hover:bg-[var(--app-accent)]/10">查看本次执行详情</button>}{events.length > 0 ? <ol className="space-y-0">{events.map((event, index) => <li key={event.id} className="relative flex gap-3 pb-5 last:pb-0"><span className={`relative z-10 mt-0.5 h-2.5 w-2.5 shrink-0 rounded-full ring-4 ring-[var(--app-surface)] ${statusColor[event.status]}`} />{index < events.length - 1 && <span className="absolute left-[5px] top-3 h-full w-px bg-[var(--app-border)]" />}<div className="min-w-0"><div className="text-xs leading-5 ui-text-soft">{event.agentName && <span className="font-medium ui-accent">{event.agentName} · </span>}{event.activity}</div><div className="mt-0.5 text-[11px] ui-dim">{statusLabel[event.status]} · {new Date(event.createdAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</div>{event.content && event.status !== 'streaming_response' && <div className="mt-1 line-clamp-2 text-[11px] leading-5 ui-muted">{event.content}</div>}</div></li>)}</ol> : <div className="rounded-xl border border-dashed ui-border p-4 text-xs leading-5 ui-dim">发送消息后，这里会显示执行进度与结果。</div>}</section>{latest?.startedAt && <div className="mt-6 border-t ui-border pt-4 text-xs ui-muted">已用时间<span className="ml-2 font-medium ui-text-soft">{String(Math.floor(elapsed / 60)).padStart(2, '0')}:{String(elapsed % 60).padStart(2, '0')}</span></div>}</>}
  </aside>;
}
