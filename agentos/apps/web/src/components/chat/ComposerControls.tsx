...
function Picker({ label, value, displayValue, options, disabled, widthClass, ariaLabel, onChange }: PickerProps) {
  ...
  return <div ref={rootRef} className="relative min-w-0">
    ...
    {open && <div role="listbox" aria-label={ariaLabel} className={`ui-panel-raised absolute bottom-full right-0 ${uiLayerClass('localMenu')} mb-2 overflow-hidden rounded-xl border p-1.5 shadow-[var(--app-shadow)] ${widthClass === 'max-w-[13rem]' ? 'w-[min(22rem,calc(100vw-2rem))]' : 'w-32'}`}>
...
function EffortPicker({ value, efforts, disabled, onChange }: EffortPickerProps) {
  ...
  return <div ref={rootRef} className="relative min-w-0">
    ...
    {open && <div role="dialog" aria-label="思考强度设置" className={`effort-popover ui-panel-raised absolute bottom-full right-0 ${uiLayerClass('localMenu')} mb-2 rounded-xl border p-3 shadow-[var(--app-shadow)]`}>
