    ...
    <form onSubmit={event => { event.preventDefault(); submit(); }} className="ui-panel-raised max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-2xl border p-5 shadow-[var(--app-shadow)] sm:p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="block text-sm ui-text-soft"><CompactSelect label="Provider" value={provider} options={[{ value: 'codex', label: 'Codex' }, { value: 'kimi', label: 'Kimi' }, { value: 'opencode', label: 'OpenCode' }, { value: 'mimo', label: 'MiMo' }, { value: 'custom', label: 'Custom' }]} onChange={value => setProvider(value as AgentProvider)} /><span className="mt-1 block text-xs ui-muted">Provider 是运行时身份；职责仍单独由上方字段定义。</span></div>
        ...
        <div className="block text-sm ui-text-soft"><CompactSelect label="默认模型" value={modelChoice} options={[{ value: '', label: '使用 CLI 默认模型' }, ...modelOptions.map(model => ({ value: model.id, label: model.label !== model.id ? model.label + ' · ' + model.id : model.label })), { value: '__custom__', label: '自定义模型' }]} onChange={nextChoice => { const nextOption = modelOptions.find(model => model.id === nextChoice); setModelChoice(nextChoice); if (nextChoice !== '__custom__') setCustomModel(''); if (nextOption && !nextOption.thinkingEfforts.includes(thinkingEffort)) setThinkingEffort(nextOption.defaultThinkingEffort); }} />{source && <span className="mt-2 flex items-center justify-between text-xs ui-muted"><span>来源：{sourceLabels[source]}</span>{onRefreshModels && <button type="button" onClick={onRefreshModels} disabled={refreshingModels} className="ui-accent disabled:opacity-50">{refreshingModels ? '刷新中…' : '刷新模型列表'}</button>}</span>}{agent.capability?.modelSourceWarning && <span className="mt-1 block text-xs text-[var(--app-warning)]">{agent.capability.modelSourceWarning}</span>}</div>
        ...
        <div className="block text-sm ui-text-soft"><CompactSelect label="默认思考强度" value={thinkingEffort} options={availableThinkingEfforts.map(effort => ({ value: effort, label: effortLabels[effort] }))} onChange={value => setThinkingEffort(value as ThinkingEffort)} />{availableThinkingEfforts.length === 1 && <span className="mt-1 block text-xs ui-muted">当前模型或 CLI 不支持可调思考强度。</span>}</div>
