...
export function GroupCreator({ agents, saving, onClose, onCreate }: GroupCreatorProps) {
  ...
  return <ModalShell title="创建协作群聊" eyebrow="GROUP SETUP" description="选择成员、角色和调度策略；保存后可在群聊中继续调整。" onClose={onClose} size="lg" footer={<div className="flex justify-end gap-3"><button type="button" onClick={onClose} className="ui-button-secondary rounded-xl px-4 py-2 text-sm">取消</button><button type="submit" form="group-creator-form" disabled={!canSubmit} className="ui-button-primary rounded-xl px-4 py-2 text-sm font-medium disabled:cursor-not-allowed">{saving ? '创建中…' : '创建群聊'}</button></div>}>
    <form id="group-creator-form" onSubmit={event => { event.preventDefault(); if (canSubmit) submit(); }}>
      <button type="button" onClick={applyStandardTeam} disabled={!enabled.some(agent => agent.role === 'codex') || !enabled.some(agent => agent.role === 'kimi') || !enabled.some(agent => agent.role === 'opencode')} className="mb-5 w-full rounded-xl border border-[color:var(--app-accent)]/40 bg-[var(--app-accent-soft)] px-3 py-3 text-left text-sm ui-text transition hover:border-[var(--app-accent)] disabled:cursor-not-allowed disabled:opacity-50"><span className="font-medium">标准开发团队</span><span className="mt-1 block text-xs ui-muted">Codex 规划 → Kimi 执行 → OpenCode 审查</span></button>
      ...
      <label className="mt-4 block text-sm ui-text-soft">调度策略<select value={dispatchMode} onChange={event => setDispatchMode(event.target.value as GroupDispatchMode)} className="ui-input mt-2 w-full rounded-xl px-3 py-2 text-sm outline-none"><option value="leader_route">Leader 路由（默认）</option><option value="full_pipeline">完整流水线</option><option value="mentioned_only">仅 @Agent</option></select></label>
      <fieldset className="mt-5"><legend className="text-sm ui-text-soft">成员与显式角色</legend><div className="mt-2 space-y-2">{enabled.map((agent, index) => { const selected = memberAgentIds.includes(agent.id); return <div key={agent.id} className="ui-panel rounded-xl border px-3 py-2 text-sm ui-text-soft"><div className="flex items-center gap-3"><input type="checkbox" aria-label={`选择 ${agent.name}`} checked={selected} onChange={() => toggle(agent.id)} className="accent-[var(--app-accent)]" />{agent.name}<span className="ml-auto text-xs ui-muted">#{index + 1}</span></div>{selected && <div className="mt-2 grid gap-2 sm:grid-cols-2"><select aria-label={`${agent.name} 角色`} value={roles[agent.id] ?? 'worker'} onChange={event => setRole(agent.id, event.target.value as CollaborationRole)} className="ui-input rounded-lg px-2 py-1 text-xs outline-none">{Object.entries(ROLE_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input aria-label={`${agent.name} 角色标题`} value={roleTitles[agent.id] ?? ''} onChange={event => setRoleTitles(current => ({ ...current, [agent.id]: event.target.value }))} placeholder="角色标题" className="ui-input rounded-lg px-2 py-1 text-xs outline-none" /></div>}</div>; })}</div></fieldset>
      <p className="mt-3 text-xs ui-muted">必须选择至少两个成员，并且只能有一个 Leader。Provider 只代表运行时，不会自动推断角色。</p>
    </form>
  </ModalShell>;
}
