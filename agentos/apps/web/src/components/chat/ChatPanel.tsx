  ...
  return <main data-signal-chat className="signal-chat flex min-w-0 flex-1 flex-col bg-[var(--app-bg)]">
    ...
    {target.kind === 'none' ? <div className="signal-empty m-6 grid flex-1 place-items-center px-6 text-center" style={{ marginTop: chromeTop + 24 }}><div className="relative z-10"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[var(--app-accent-soft)] text-2xl ui-accent">✦</div><p className="mt-4 text-sm ui-muted">从左侧选择一个 Agent 或群聊。</p></div></div> : <>
      ...
      <div className="signal-composer-shell border-t ui-border px-4 py-4 sm:px-6"><div ref={composerGlassRef} className="signal-composer mx-auto max-w-4xl rounded-2xl border ui-border bg-[var(--app-surface-raised)] p-3 transition focus-within:border-[var(--app-accent)]" onPaste={event => { const imageFiles = Array.from(event.clipboardData.items).filter(isImageClipboardItem).map(item => item.getAsFile()).filter((file): file is File => Boolean(file)); if (imageFiles.length > 0) { event.preventDefault(); onFiles(imageFiles); } }}>
        ...
        <div className="composer-action-row mt-2 flex items-center justify-between gap-3"><ImageAttachments drafts={attachments} disabled={sending} onFiles={onFiles} onRemove={onRemoveAttachment} /><span className="composer-hint hidden text-xs ui-dim sm:inline">{sending ? 'Enter 加入队列 · Shift + Enter 换行' : 'Enter 发送 · Shift + Enter 换行'}</span><div className="ml-auto flex min-w-0 items-center gap-2"><ComposerControls isGroup={isGroup} modelOptions={modelOptions} model={composerModel} thinkingEffort={composerThinkingEffort} thinkingEfforts={composerThinkingEfforts} modelSource={modelSource} disabled={sending} onModelChange={onComposerModelChange} onThinkingEffortChange={onComposerThinkingEffortChange} runIntent={runIntent} onRunIntentChange={onRunIntentChange} />{sending && <button type="button" onClick={onCancel} title="中断当前运行" aria-label="中断执行" className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-[color:var(--app-danger)]/60 bg-[color:var(--app-danger)]/15 text-[var(--app-danger)] transition hover:bg-[color:var(--app-danger)]/25 active:scale-95"><span aria-hidden="true" className="h-2.5 w-2.5 rounded-[2px] bg-[var(--app-danger)]" /></button>}<button type="button" onClick={onSend} disabled={sendButtonState.disabled} aria-busy={sendButtonState.ariaBusy} aria-label={sendButtonState.label} className="ui-button-primary grid h-9 w-9 shrink-0 place-items-center rounded-xl text-lg disabled:cursor-not-allowed disabled:opacity-50">{sendButtonState.showSpinner ? <span aria-hidden="true" className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" /> : <span aria-hidden="true">↑</span>}</button></div></div>
      </div></div></div>
    </>}
  </main>;
}
