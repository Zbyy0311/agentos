...
const MessageContent = memo(function MessageContent({ content }: { content: string }) {
  ...
  const collapsible = content.length > 4000 || lineCount > 40;
  const [expanded, setExpanded] = useState(false);
  if (!collapsible) return <MarkdownMessage content={content} />;
  return <div className="long-message">
    <div className={expanded ? undefined : 'long-message-collapsed'}><MarkdownMessage content={content} /></div>
    <button type="button" className="long-message-toggle" aria-expanded={expanded} onClick={() => setExpanded(current => !current)}>{expanded ? '收起全文' : '展开全文'}</button>
  </div>;
});
...
const MessageBubble = memo(function MessageBubble({ message, agent, isGroup }: { message: ConversationMessage; agent?: AgentProfile; isGroup: boolean }) {
  const userMessage = message.senderType === 'user';
  return <div className={`flex gap-3 ${userMessage ? 'justify-end' : 'justify-start'}`}><div className={`signal-message ${userMessage ? 'ui-message-user order-2' : message.senderType === 'system' ? 'ui-message-system' : 'ui-message-agent'} max-w-[86%] min-w-0 rounded-2xl border px-4 py-3 text-sm leading-6 sm:max-w-[78%]`}>{isGroup && agent && <div className="mb-2 border-b ui-border pb-2 text-xs font-medium ui-accent">{agent.name}<span className="ml-1 font-normal ui-muted">· {agent.roleTitle}</span></div>}<MessageContent content={message.content} /><MessageAttachments attachments={message.attachments} /></div></div>;
});
...
export function ChatPanel({ agentName, roleTitle, conversationTitle, groupName, isGroup = false, agents, messages, draft, attachments, attachmentError, streamingContent, activeEvents, activeRuntimeEvents = [], artifacts = [], apiBase = '', activeStatus, waitingQuestion, connectionNotice, validationError, error, sending, queuedMessageCount, modelOptions, composerModel, composerThinkingEffort, composerThinkingEfforts, modelSource, onDraftChange, onFiles, onRemoveAttachment, onComposerModelChange, onComposerThinkingEffortChange, onSend, onCancel, onRename, mentionedAgentIds = [], onMentionedAgentIdsChange, runIntent = 'execute', onRunIntentChange = value => window.dispatchEvent(new CustomEvent('agentos:run-intent', { detail: value })) }: ChatPanelProps) {
  ...
  const renderMessage = (message: ConversationMessage) => <MessageBubble key={message.id} message={message} agent={message.senderAgentId ? agentMap.get(message.senderAgentId) : undefined} isGroup={isGroup} />;
  ...
  return <main data-signal-chat className="signal-chat flex min-w-0 flex-1 flex-col bg-[var(--app-bg)]">
    ...
    {target.kind === 'none' ? <div className="signal-empty m-6 grid flex-1 place-items-center px-6 text-center"><div className="relative z-10"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[var(--app-accent-soft)] text-2xl ui-accent">✦</div><p className="mt-4 text-sm ui-muted">从左侧选择一个 Agent 或群聊。</p></div></div> : <>
      <div ref={scrollRef} onScroll={event => { wasNearBottomRef.current = isNearBottom(event.currentTarget); }} className="signal-chat-scroll flex-1 overflow-y-auto px-4 py-7 sm:px-6"><div className="mx-auto max-w-4xl space-y-5">
        ...
        {messages.length <= 100 && messages.map(message => renderMessage(message))}
        ...
        {status && <StatusNotice tone={activeStatus === 'waiting_user' ? 'waiting' : 'info'}>{status}</StatusNotice>}
        {connectionNotice && <StatusNotice tone="warning">{connectionNotice}</StatusNotice>}
        {activeStatus === 'waiting_user' && waitingQuestion && <StatusNotice tone="waiting" title="Agent 需要补充信息">{waitingQuestion}</StatusNotice>}
        {error && <StatusNotice tone="error" title="执行失败">{error}</StatusNotice>}
