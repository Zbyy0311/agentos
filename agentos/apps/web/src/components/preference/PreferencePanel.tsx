...
export function PreferencePanel({ workspaceId, onClose, onOpenRun }: PreferencePanelProps) {
  ...
  return <div className={`fixed inset-0 ${uiLayerClass('workspaceSurface')} bg-[var(--app-surface)] p-6`}><div className="mx-auto flex h-full max-w-5xl flex-col"><div className="mb-5 flex items-start justify-between gap-4"><div><div className="text-[11px] tracking-[0.16em] ui-dim">ADAPTIVE PREFERENCES</div><h2 className="mt-1 text-xl font-semibold ui-text">交互与工作偏好</h2><p className="mt-1 text-xs leading-5 ui-muted">只根据你的实际操作和修正逐步学习；当前要求始终优先。</p></div><button type="button" onClick={onClose} className="ui-button-ghost rounded-lg px-3 py-2 text-sm">返回聊天</button></div>
