...
import type { CliEventParser, NormalizedCliEvent } from './types.js';

type JsonRecord = Record<string, unknown>;

/** Child-process-free Codex JSONL parser shared by legacy and canonical adapters. */
export class CodexJsonParser implements CliEventParser {
  ...
  private mapRecord(record: JsonRecord): NormalizedCliEvent[] {
    ...
    switch (eventType) {
      case 'thread.started': return [{ type: 'status', phase: 'starting', label: 'Codex 已启动' }];
      case 'turn.started': return [{ type: 'status', phase: 'thinking', label: '正在分析任务' }];
      case 'turn.completed': return [{ type: 'status', phase: 'finalizing', label: '正在整理结果' }, ...this.mapUsage(record.usage)];
      case 'turn.failed': return [{ type: 'diagnostic', level: 'error', code: 'adapter.turn_failed', message: 'Codex turn 执行失败' }];
      case 'error': return [{ type: 'diagnostic', level: 'error', code: 'adapter.cli_error', message: 'Codex CLI 返回错误' }];
      case 'item.started': return this.mapItem(record.item, 'started');
      case 'item.completed': return this.mapItem(record.item, 'completed');
      default: return [unknownEventDiagnostic()];
    }
  }
  ...
  private mapItem(value: unknown, phase: 'started' | 'completed'): NormalizedCliEvent[] {
    ...
    if (itemType === 'reasoning') return [{ type: 'status', phase: 'thinking', label: '正在分析任务' }];
