...
try {
  ...
  & node (Join-Path $repoRoot 'scripts/verify-real-codex-runtime-e2e.mjs')
  if ($LASTEXITCODE -ne 0) { throw "Real Codex AgentOS gate failed with exit code $LASTEXITCODE" }
} finally {
