/**
 * Build the strict local-tests receipt consumed by verify-lite-scope.mjs.
 * A run is accepted only when its original TAP summary is complete and every
 * declared requirement is mapped to a named, executed source assertion.
 */
...
const countFor = marker => {
  const matches = [...output.matchAll(new RegExp(`^# ${marker} (\\d+)\\s*$`, 'gm'))];
  if (matches.length !== 1) throw new Error(`TAP summary ${marker} is missing or ambiguous`);
  return Number(matches[0][1]);
};
const counts = { passed: countFor('pass'), failed: countFor('fail'), skipped: countFor('skipped') };
...
const assertionCoverage = assertions.map(assertion => {
  ...
  if (!sourceLine?.includes(assertion.expression)) throw new Error(`expression is not at source line: ${assertion.id}`);
  if (!sourceText.includes(assertion.name)) throw new Error(`test name is absent from source: ${assertion.id}`);
  if (!output.split(/\r?\n/).some(line => /^\s*ok \d+ - /.test(line) && line.includes(assertion.name))) {
    throw new Error(`test name has no executed passing TAP result: ${assertion.id}`);
  }
  return { ...assertion };
});
